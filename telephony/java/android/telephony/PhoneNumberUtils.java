    ...
    private static String preProcessPlusCode(String dialStr) {
        if (!TextUtils.isEmpty(dialStr)) {
            if (isReallyDialable(dialStr.charAt(0)) &&
                ...
                if (needToFormat) {
                    Rlog.d(LOG_TAG, "preProcessPlusCode, before format number:" + dialStr);
                    String result = sPlusCodeUtils.replacePlusCodeWithIddNdd(dialStr);
                    if (result != null && result.length() > 0) {
                        dialStr = result;
                    }
                    Rlog.d(LOG_TAG, "preProcessPlusCode, after format number:" + dialStr);
                    return dialStr;
                } else {
                    Rlog.d(LOG_TAG, "preProcessPlusCode, no need format, currIso:" + currIso
