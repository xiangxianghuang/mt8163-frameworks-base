...
int BootAnimation::getSystemVolume()
{
    char value[PROPERTY_VALUE_MAX];
    property_get("persist.sys.volume", value, "11");
    int volume = atoi(value);
    ALOGE("YYD.service.settings.volume: %d\n", volume);
    return volume;
}
...
bool BootAnimation::threadLoop()
{
    ...
    if (resourcePath != NULL) {
        ...
        if (mediastatus == NO_ERROR) {
            // @{ Read music volume from build.prop. /* By Bright.L 2017.1 */
            int mSystemVolume = getSystemVolume();  
            float mVolume = mSystemVolume / 15.0;
            ALOGE("YYD.mediaplayer.mVolume: %f\n" , mVolume);
