...
public final class ViewRootImpl implements ViewParent,
        ...
        synchronized (this) {
            if (mView == null) {
                ...
                if (DEBUG_LAYOUT || DEBUG_LIFECYCLE) {
                    Log.d(TAG, "WindowLayout in setView:" + attrs + ",mView = " + mView
                ...
                if (DEBUG_LAYOUT) {
                    Log.v(TAG, "Added window " + mWindow + ", mPendingContentInsets = "
    ...
    private void enableHardwareAcceleration(WindowManager.LayoutParams attrs) {
        ...
        if (!hardwareAccelerated) {
            Log.d(TAG, "disable hardware acceleration by APP!!!, this = " + this);
        }

        if (hardwareAccelerated) {
            if (!HardwareRenderer.isAvailable()) {
                /// M : System doesn't support hardware acceleration
                Log.d(TAG, "system doesn't support hardware acceleration!!!, this = " + this);
                return;
            }
            ...
            if (fakeHwAccelerated) {
                ...
                if (mAttachInfo.mHardwareRenderer != null) {
                    ...
                    Log.d(TAG, "hardware acceleration is enabled, this = " + this);
                }
            }
            /// M : Hardware acceleration is disabled due to following reasons.
            if (mAttachInfo.mHardwareRenderer == null) {
                 Log.d(TAG, "hardware acceleration is disabled, fakeHwAccelerated = "
                        + fakeHwAccelerated + ", HardwareRenderer.sRendererDisabled = "
                        + HardwareRenderer.sRendererDisabled + ", forceHwAccelerated = "
                        + forceHwAccelerated + ", HardwareRenderer.sSystemRendererDisabled = "
                        + HardwareRenderer.sSystemRendererDisabled + ", this = " + this);
            }
        }
    }
    ...
    void setLayoutParams(WindowManager.LayoutParams attrs, boolean newView) {
        ...
        if (DEBUG_IMF) {
            Log.d(TAG, "setLayoutParams: attrs = " + attrs + ", mSoftInputMode = " + mSoftInputMode
    ...
    void handleAppVisibility(boolean visible) {
        if (DEBUG_DRAW || DEBUG_LAYOUT) {
            Log.d(TAG, "handleAppVisibility: visible=" + visible + ", mAppVisible=" + mAppVisible
                    + ", this = " + this);
        }
    ...
    public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
        checkThread();
        if (DEBUG_DRAW) {
            Log.v(TAG, "Invalidate child: " + dirty + ",this = " + this);
        }
    ...
    private void invalidateRectOnScreen(Rect dirty) {
        ...
        if (!mWillDrawSoon && (intersected || mIsAnimating)) {
            ...
            if (DEBUG_DRAW) {
                Log.v(TAG, "Invalidate child: Do not scheduleTraversals, mWillDrawSoon ="
    ...
    void setWindowStopped(boolean stopped) {
        if (DEBUG_LAYOUT) {
            Log.v(TAG, "setWindowStopped: stopped = " + stopped + ", this = " + this);
        }
    ...
    public boolean getChildVisibleRect(View child, Rect r, android.graphics.Point offset) {
        ...
        if (DEBUG_DRAW) {
            Log.v(TAG, "getChildVisibleRect: child = " + child + ",r = " + r + ", this = " + this);
        }
    ...
    void scheduleTraversals() {
        if (!mTraversalScheduled) {
            ...
            if (DEBUG_LIFECYCLE) {
                Log.v(TAG, "scheduleTraversals: mTraversalBarrier = " + mTraversalBarrier
                        + ",this = " + this);
            }
    ...
    void unscheduleTraversals() {
        if (DEBUG_LIFECYCLE) {
            Log.v(TAG, "unscheduleTraversals: mTraversalScheduled = " + mTraversalScheduled
    ...
    void doTraversal() {
        if (DEBUG_LIFECYCLE) {
            Log.v(TAG, "doTraversal: mTraversalScheduled = " + mTraversalScheduled
                    + ",mTraversalBarrier = " + mTraversalBarrier + ",this = " + this);
        }
    ...
    private boolean measureHierarchy(final View host, final WindowManager.LayoutParams lp,
        ...
        if (DEBUG_ORIENTATION || DEBUG_LAYOUT || DEBUG_MEASURE_LAYOUT) {
            Log.v(TAG, "ViewRoot measure+ : " + host + " in display " + desiredWindowWidth + "x"
        ...
        if (lp.width == ViewGroup.LayoutParams.WRAP_CONTENT) {
            ...
            /// M: BMW. [ALPS02530232] Don't use baseSize to layout dialog which
            /// is in floating stack. Because the basesize is for full screen case.@{
            if (MultiWindowProxy.isSupported()) {
                if (MultiWindowProxy.getInstance().isFloatingByWinToken(host.getWindowToken())) {
                    baseSize = 0;
                }
            }
            /// @}
            if (DEBUG_DIALOG) {
                Log.v(TAG, "Window " + mView + ": baseSize=" + baseSize + ", this = " + this);
            }
            if (baseSize != 0 && desiredWindowWidth > baseSize) {
                ...
                if (DEBUG_DIALOG) {
                    Log.v(TAG, "Window " + mView + ": measured (" + host.getMeasuredWidth() + ","
                ...
                if ((host.getMeasuredWidthAndState()&View.MEASURED_STATE_TOO_SMALL) == 0) {
                    ...
                    if (DEBUG_DIALOG) {
                        Log.v(TAG, "Window " + mView + ": next baseSize=" + baseSize + " " + this);
                    }
                    childWidthMeasureSpec = getRootMeasureSpec(baseSize, lp.width);
                    performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
                    if (DEBUG_DIALOG) {
        ...
        if (DEBUG_ORIENTATION || DEBUG_LAYOUT || DEBUG_MEASURE_LAYOUT) {
            Log.v(TAG, "ViewRoot measure-: host measured size = (" + host.getMeasuredWidth()
    ...
    private void performTraversals() {
        ...
        if (DEBUG_LIFECYCLE || DEBUG_DEFAULT) {
            Log.v(TAG, "ViewRoot performTraversals+ : mFirst = " + mFirst + ", this = " + this);
        }
        ...
        boolean viewVisibilityChanged = mViewVisibility != viewVisibility
                || mNewSurfaceNeeded || mViewVisibilityChanged;
        ...
        if (mFirst) {
            ...
            if (desiredWindowWidth != mWidth || desiredWindowHeight != mHeight) {
                if (DEBUG_ORIENTATION || DEBUG_LAYOUT) {
                    Log.v(TAG, "View " + host + " resized to: " + frame + ",this = " + this);
                }
                mFullRedrawNeeded = true;
                mLayoutRequested = true;
                windowSizeMayChange = true;
        ...
        if (layoutRequested) {
            ...
            if (mFirst) {
                ...
                if (!mPendingContentInsets.equals(mAttachInfo.mContentInsets)) {
                    if (DEBUG_LAYOUT) {
                        Log.v(TAG, "Content insets changing from " + mPendingContentInsets
        ...
        if (mFirst || mAttachInfo.mViewVisibilityChanged) {
            ...
            if (DEBUG_INPUT_RESIZE) {
                Log.v(TAG, "Handle input resize mode start+ : lp.softInputMode = #"
            ...
            if (DEBUG_INPUT_RESIZE) {
                Log.v(TAG, "Handle input resize mode end- : lp.softInputMode = #"
                        + Integer.toHexString(lp.softInputMode) + ", resizeMode = #"
                        + Integer.toHexString(resizeMode) + ", this = " + this);
            }
        }
        ...
        if (DEBUG_LAYOUT) {
            Log.v(TAG, "ViewRoot adjustSize+ : mFirst=" + mFirst
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
                ...
                if (!hadSurface) {
                    ...
                    if (DEBUG_DRAW) {
                        Log.v(TAG, "Surface invalid: " + mSurface + ",surfaceGenerationId = "
                ...
                Log.e(TAG, "RemoteException happens in " + this, e);
            }
        ...
        if (DEBUG_LAYOUT) {
            Log.v(TAG, "ViewRoot adjustSize- " + ", frame = " + frame
        ...
        if (didLayout) {
            ...
            if ((host.mPrivateFlags & View.PFLAG_REQUEST_TRANSPARENT_REGIONS) != 0) {
                ...
                if (DEBUG_DRAW || DBG_TRANSP) {
                    Log.v(TAG, "ViewRoot gatherTransparentRegion+ : mTransparentRegion = "
                ...
                if (!mTransparentRegion.equals(mPreviousTransparentRegion)) {
                    ...
                    try {
                        ...
                        Log.e(TAG, "Exception in " + this + " when set transparent region.", e);
                    }
                }
            }
        ...
        if (computesInternalInsets) {
            ...
            if (insetsPending || !mLastGivenInsets.equals(insets)) {
                ...
                try {
                    ...
                    Log.e(TAG, "RemoteException happens when setInsets, mWindow = " + mWindow
        ...
        if (mFirst) {
            ...
            if (DEBUG_INPUT_RESIZE) {
                Log.v(TAG, "First: mView.hasFocus()=" + (mView != null ? mView.hasFocus() : "null")
        ...
        if (DEBUG_DRAW) {
            Log.v(TAG, "ViewRoot draw+ " + ", cancelDraw=" + cancelDraw
        ...
        if (DEBUG_DRAW || DEBUG_LIFECYCLE || DEBUG_DEFAULT) {
            ...
            Log.v(TAG, "ViewRoot performTraversals and draw- : mResizeBuffer = " + mResizeBuffer
    ...
    private void performLayout(WindowManager.LayoutParams lp, int desiredWindowWidth,
        ...
        final View host = mView;
        if (DEBUG_ORIENTATION || DEBUG_LAYOUT) {
            Log.v(TAG, "Laying out " + host + " in " + this + " to (" +
    ...
    public void onHardwarePostDraw(DisplayListCanvas canvas) {
        if (DEBUG_DRAW) {
            Log.v(TAG, "onHardwarePostDraw: mResizeBuffer = " + mResizeBuffer + ", mResizeAlpha = "
                    + mResizeAlpha + ", this = " + this);
        }
        if (mResizeBuffer != null) {
    ...
    private void performDraw() {
        ...
        if (mReportNextDraw) {
            ...
            try {
                ...
                Log.e(TAG, "Exception when finish draw window " + mWindow + " in " + this, e);
            }
            Trace.traceEnd(Trace.TRACE_TAG_VIEW);
        }
    }
        ...
        if (!dirty.isEmpty() || mIsAnimating || accessibilityFocusDirty) {
            if (mAttachInfo.mHardwareRenderer != null && mAttachInfo.mHardwareRenderer.isEnabled()) {
                ...
                if (!drawSoftware(surface, mAttachInfo, xOffset, yOffset, scalingRequired, dirty)) {
                    ...
                    if (DEBUG_DRAW) {
                        Log.v(TAG, "drawSoftware return: this = " + this);
                    }
    ...
    private boolean drawSoftware(Surface surface, AttachInfo attachInfo, int xoff, int yoff,
        ...
        try {
            ...
            if (DEBUG_DRAW) {
                ...
                Log.i(TAG, "Drawing view start+ : package:" + cxt.getPackageName() +
            ...
            if (DEBUG_DRAW) {
                Log.v(TAG, "Drawing view end- : mView = " + mView + ", this = " + this);
            }
        } finally {
            try {
                surface.unlockCanvasAndPost(canvas);
            } catch (IllegalArgumentException e) {
                Log.e(TAG, "Could not unlock surface, surface = " + surface
            ...
            if (DEBUG_DRAW) {
                Log.v(TAG, "Surface " + surface + " unlockCanvasAndPost, this = " + this);
            }
        }
        return true;
    }
    ...
    boolean scrollToRectOrFocus(Rect rectangle, boolean immediate) {
        ...
        if (vi.left > ci.left || vi.top > ci.top
            ...
            if (DEBUG_INPUT_RESIZE) {
                Log.v(TAG, "Eval scroll: focus=" + focus + " rectangle=" + rectangle + " ci=" + ci
            ...
            if (focus == lastScrolledFocus && !mScrollMayChange && rectangle == null) {
                ...
                if (DEBUG_INPUT_RESIZE) {
                    Log.v(TAG, "Need to scroll? mVisRect = " + mVisRect + " this = " + this);
                }
                // Try to find the rectangle from the focus view.
                if (focus.getGlobalVisibleRect(mVisRect, null)) {
                    if (DEBUG_INPUT_RESIZE) {
                        Log.v(TAG, "Root w=" + mView.getWidth() + " h=" + mView.getHeight()
                    ...
                    if (rectangle == null) {
                        ...
                        if (DEBUG_INPUT_RESIZE) {
                            Log.v(TAG, "Focus " + focus + ": focusRect="
                        ...
                        if (DEBUG_INPUT_RESIZE) {
                            Log.v(TAG, "Focus in window: focusRect=" + mTempRect.toShortString()
                                    + " visRect=" + mVisRect.toShortString() + " this = " + this);
                        }
                    } else {
                        mTempRect.set(rectangle);
                        if (DEBUG_INPUT_RESIZE) {
                            Log.v(TAG, "Request scroll to rect: " + mTempRect.toShortString()
                                    + " visRect=" + mVisRect.toShortString() + " this = " + this);
                        }
                    }
                    if (mTempRect.intersect(mVisRect)) {
                        ...
                        if (mTempRect.height() >
                            ...
                            if (DEBUG_INPUT_RESIZE) {
                                Log.v(TAG, "Top covered; scrollY=" + scrollY + " this = " + this);
                            }
    ...
    public void requestChildFocus(View child, View focused) {
        if (DEBUG_INPUT_RESIZE) {
            Log.v(TAG, "Request child " + child + " focus: focus now " + focused + " in " + this);
        }
        checkThread();
        scheduleTraversals();
    }
    ...
    void dispatchDetachedFromWindow() {
        ...
        try {
            ...
            Log.e(TAG, "RemoteException remove window " + mWindow + " in " + this, e);
        }
    ...
    void updateConfiguration(Configuration config, boolean force) {
        if (DEBUG_CONFIGURATION) {
            Log.v(TAG, "Applying new config to window " + mWindowAttributes.getTitle() + ": "
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_RESIZED_REPORT:
                if (mAdded) {
                    ...
                    if (DEBUG_LAYOUT) {
                        Log.d(TAG, "Handle RESIZE: message = " + msg.what + " ,this = "
                                + ViewRootImpl.this);
                    }
                        ...
                        if (mAttachInfo.mHardwareRenderer != null && mSurface.isValid()){
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    Log.e(TAG, "RemoteException when call outOfMemory.", e);
                                }
                                // Retry in a bit.
    ...
    abstract class InputStage {
        ...
        void dumpStageInfo(SimpleDateFormat sdf) {
            ...
            if (mDeliveredTimestamp != 0) {
                Log.v(TAG, "Input event delivered to " + this + " at "
    ...
    final class ImeInputStage extends AsyncInputStage
            ...
            if (mLastWasImTarget && !isInLocalFocusMode()) {
                ...
                if (imm != null) {
                    ...
                    if (DEBUG_IMF || DEBUG_INPUT || DEBUG_KEY) {
                        Log.v(TAG, "Sending input event to IME: " + event + ", this = " + this);
                    }
        ...
        public void onFinishedInputEvent(Object token, boolean handled) {
            QueuedInputEvent q = (QueuedInputEvent)token;
            if (DEBUG_IMF || DEBUG_INPUT || DEBUG_KEY) {
                Log.d(TAG, "IME finishedEvent: handled = " + handled + ", event = " + q
                        + ",viewAncestor = " + this);
            }
            if (handled) {
                finish(q, true);
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        private int processKeyEvent(QueuedInputEvent q) {
            ...
            if (DEBUG_KEY || DEBUG_INPUT) {
                Log.v(TAG, "deliverKeyEventPostIme: event = " + event + ", mView = " + mView
            ...
            if (mView.dispatchKeyEvent(event)) {
                if (DEBUG_DEFAULT) {
                    Log.v(TAG, "App handle key event: event = " + event + ", mView = " + mView
                            + ", this = " + this);
                }
                return FINISH_HANDLED;
            }
        ...
        private int processPointerEvent(QueuedInputEvent q) {
            ...
            if (handled && DEBUG_DEFAULT) {
                Log.v(TAG, "App handle pointer event: event = " + event + ", mView = " + mView
                        + ", this = " + this);
            }
    ...
    private void handleDragEvent(DragEvent event) {
        if (DEBUG_INPUT) {
            Log.v(TAG, "handleDragEvent: event = " + event + ", mView = " + mView + ", mAdded = "
                    + mAdded + ", this = " + this);
        }
    ...
    public void handleDispatchWindowAnimationStarted(int remainingFrameCount) {
        if (DEBUG_DRAW) {
            Log.v(TAG, "handleDispatchWindowAnimationStarted: mDrawDuringWindowsAnimating = "
                    + mDrawDuringWindowsAnimating + ", remainingFrameCount = " + remainingFrameCount
                    + ", this = " + this);
        }
    ...
    public void handleDispatchWindowAnimationStopped() {
        if (DEBUG_DRAW) {
            Log.v(TAG, "handleDispatchWindowAnimationStopped: mDirty = " + mDirty
                    + ", mWindowsAnimating = " + mWindowsAnimating + ", mIsAnimating = "
                    + mIsAnimating + ", this = " + this);
        }
    ...
    private AudioManager getAudioManager() {
        ...
        if (mAudioManager == null) {
            mAudioManager = (AudioManager) mView.getContext().getSystemService(
    ...
    public boolean performHapticFeedback(int effectId, boolean always) {
        try {
            ...
            Log.e(TAG, "performHapticFeedback RemoteException happens in " + this, e);
            return false;
        }
    }
    ...
    boolean die(boolean immediate) {
        if (DEBUG_LIFECYCLE) {
            Log.v(TAG, "die: immediate = " + immediate + ", mIsInTraversal = " + mIsInTraversal
    ...
    void doDie() {
        ...
        checkThread();
        if (DEBUG_LIFECYCLE) {
            Log.v(TAG, "DIE in " + this + " of " + mSurface + ",mAdded = " + mAdded
                    + ",mFirst = " + mFirst);
        }
                ...
                if (mView != null) {
                    ...
                    if (mWindowAttributesChanged || viewVisibilityChanged) {
                        ...
                        try {
                            ...
                            Log.e(TAG, "RemoteException when finish draw window " + mWindow
                                    + " in " + this, e);
                        }
                    }
    ...
    private void destroyHardwareRenderer() {
        ...
        if (DEBUG_HWUI && isTargetPackage()) {
            Log.v(TAG, "destroyHardwareRenderer: hardwareRenderer = " + hardwareRenderer
                    + ",mView = " + mView + ",this = " + this);
        }
    ...
    public void dispatchResized(Rect frame, Rect overscanInsets, Rect contentInsets,
        ...
        if (DEBUG_LAYOUT) Log.v(TAG, "Resizing " + this + ": frame=" + frame.toShortString()
                ...
                + " reportDraw=" + reportDraw + ", this = " + this);
    ...
    public void dispatchMoved(int newX, int newY) {
        if (DEBUG_LAYOUT) {
            Log.v(TAG, "Window moved " + this + ": newX=" + newX + " newY=" + newY);
        }
    ...
    void enqueueInputEvent(InputEvent event,
        ...
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION || DEBUG_DEFAULT) {
            Log.v(TAG, "enqueueInputEvent: event = " + event + ",processImmediately = "
    ...
    private void scheduleProcessInputEvents() {
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Log.v(TAG, "scheduleProcessInputEvents: mPendingInputEventHead = "
    ...
    void doProcessInputEvents() {
        ...
        while (mPendingInputEventHead != null) {
            ...
            /// @}
            if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
                Log.v(TAG, "doProcessInputEvents: mCurrentInputEvent = " + q + ", this = " + this);
            }
            deliverInputEvent(q);
        }
    ...
    private void finishInputEvent(QueuedInputEvent q) {
        ...
        if (inputElapseTime >= INPUT_TIMEOUT) {
            ...
            Log.v(TAG, "[ANR Warning]Input routeing takes more than 6000ms since "
        ...
        if (DEBUG_DEFAULT || DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Log.v(TAG, "finishInputEvent: handled = " + handled + ",event = " + q.mEvent
    ...
    void scheduleConsumeBatchedInput() {
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Log.v(TAG, "scheduleConsumeBatchedInput: mConsumeBatchedInputScheduled = "
    ...
    void unscheduleConsumeBatchedInput() {
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Log.v(TAG, "unscheduleConsumeBatchedInput: mConsumeBatchedInputScheduled = "
    ...
    void doConsumeBatchedInput(long frameTimeNanos) {
        if ((mPendingInputEventHead != null) && (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION)) {
            Log.v(TAG, "doConsumeBatchedInput: frameTimeNanos = " + frameTimeNanos
    ...
    final class WindowInputEventReceiver extends InputEventReceiver {
        ...
        public void onBatchedInputEventPending() {
            if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
                Log.v(TAG, "onBatchedInputEventPending: this = " + this);
            }
            if (mUnbufferedInputDispatch) {
     ...
     */
    public void enableLog(boolean enable) {
        Log.v(TAG, "enableLog: enable = " + enable);
//        DBG = enable;
        LOCAL_LOGV = enable;
        DEBUG_DRAW = enable;
        DEBUG_LAYOUT = enable;
    ...
    public void dumpInputDispatchingStatus() {
        ...
        if (mCurrentKeyEvent == null) {
            Log.i(TAG, "ANR Key Analyze: No Key event currently.");
            Log.i(TAG, "ANR Key Analyze: Previeous Event " + mPreviousKeyEvent +  ",finish at "
                    + simpleDateFormat.format(date2));
        } else {
            Log.i(TAG, "Dispatch " + mCurrentKeyEvent + " status is " + mKeyEventStatus + ",start"
                    + " at " + simpleDateFormat.format(date) + ", spent " + dispatchTime + "ms.");
        }

        if (mCurrentMotion == null) {
            date2.setTime(mPreviousMotionEventFinishTime);
            Log.i(TAG, "ANR Motion Analyze: No motion event currently.");
            Log.i(TAG, "ANR Motion Analyze: Previeous Event " + mPreviousMotion
            ...
            Log.i(TAG, "Dispatch " + mCurrentMotion + " status is " + mMotionEventStatus + ",start"
                    + " at " + simpleDateFormat.format(date) + ", spent " + dispatchTime + "ms.");
        }
        /// M: [ANR] Add for monitoring stage status. @{
        mFirstInputStage.dumpStageInfo(simpleDateFormat);
