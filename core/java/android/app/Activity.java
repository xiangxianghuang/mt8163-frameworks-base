...
public class Activity extends ContextThemeWrapper
    ...
    public void startActivityForResult(Intent intent, int requestCode, @Nullable Bundle options) {
        /// M: BMW. Adjust intent, if current activity is floating @{
        if (MultiWindowProxy.isSupported()) {
            if (MultiWindowProxy.isFloatingIntent(mIntent)
                    || MultiWindowProxy.getInstance().isFloatingByAppToken(mToken)) {
                MultiWindowProxy.adjustFloatingIntent(intent);
            }
        }
        /// @}
        if (mParent == null) {
    ...
    public void startActivityForResult(
        ...
        /// M: BMW. [ALPS02530868] Adjust intent, if current activity is floating @{
        if (MultiWindowProxy.isSupported()) {
            if (MultiWindowProxy.isFloatingIntent(mIntent)
                    || MultiWindowProxy.getInstance().isFloatingByAppToken(mToken)) {
                MultiWindowProxy.adjustFloatingIntent(intent);
    ...
    public boolean moveTaskToBack(boolean nonRoot) {
        /// M: BMW. ALPS02078629 reset stick when moveTaskToBack. @{
        if (MultiWindowProxy.isSupported()) {
            MultiWindowProxy.getInstance().stickWindow(mToken, false);
        }
        /// @}
        try {
