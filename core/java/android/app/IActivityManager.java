...
public interface IActivityManager extends IInterface {
    ...
    /// M: dump message history and future messages for app transition timeout
    ///    or freeze timeout cases
    public void notifyWindowTimeout() throws RemoteException;
    /// M: GMO, set wallpaper process
    public void setWallpaperProcess(ComponentName className) throws RemoteException;
    /// M: GMO, update wallpaper process adj
    ...
    /// M: dump message history and future messages for app transition timeout
    ///    or app freeze timeout cases
    int NOTIFY_WINDOW_TIMEOUT = IBinder.FIRST_CALL_TRANSACTION + 400;
    /// M: GMO, set wallpaper process
    int SET_WALLPAPER_PROCESS = IBinder.FIRST_CALL_TRANSACTION + 401;
    /// M: GMO, update wallpaper process adj
    int UPDATE_WALLPAPER_STATE = IBinder.FIRST_CALL_TRANSACTION + 402;

    /// M: ALPS00431397. When running window animation, halt the activity
    /// launchigng. Otherwise, restore the activity state @{
    int HALT_ACTIVITY_RESUMING = IBinder.FIRST_CALL_TRANSACTION + 403;
    int RESTORE_ACTIVITY_RESUMING = IBinder.FIRST_CALL_TRANSACTION + 404;
    /// @}

    /// M: ALPS00404470. GMO check-in
    int GET_PROCESS_PSWAP_TRANSACTION = IBinder.FIRST_CALL_TRANSACTION + 405;
    /// @}

    /// M: ALPS01257806, Force kill AP for IPO without stopping the package @{
    int FORCE_KILL_PACKAGE_TRANSACTION = IBinder.FIRST_CALL_TRANSACTION + 406;
    /// @}

    /// M: Return the moving result for error handling in RecentApp. @{
    int MOVE_TASK_TO_FRONT_WITH_RESULT_TRANSACTION = IBinder.FIRST_CALL_TRANSACTION + 407;
    /// @}

    /// M: App-based AAL @{
    int AAL_SET_AAL_MODE = IBinder.FIRST_CALL_TRANSACTION + 408;
    int AAL_SET_AAL_ENABLED = IBinder.FIRST_CALL_TRANSACTION + 409;
    /// M: App-based AAL @}
}
