...
public class ActionBarContainer extends FrameLayout {
    ...
    public boolean gatherTransparentRegion(Region region) {
        /// M: BMW. To get the control bar height @{
        if (MultiWindowProxy.isSupported()) {
            if (mActionBarView != null
                && MultiWindowProxy.getInstance().isFloatingByWinToken(
                    mActionBarView.getWindowToken())) {
                /*mControlBarHeight = context.getResources().getDimensionPixelSize(
                        com.mediatek.internal.R.dimen.floatwindow_action_bar_height);*/
                mControlBarHeight = 44;
                mIsFloatingWindow = true;
            }
        }
        /// @}
        if (region != null) {
            /// M: BMW. [ALPS02031476] Fix gallery layout issue @{
            int offset = 0;
            if (mIsFloatingWindow) {
                offset = mControlBarHeight;
            }
            region.op(0, 0, mRight - mLeft, mBottom - mTop +
                      mStatusBarHeight + offset, Region.Op.DIFFERENCE);
            /// @}
        }
        return true;
    }
