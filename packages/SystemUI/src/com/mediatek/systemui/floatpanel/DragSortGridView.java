...
public class DragSortGridView extends GridView {
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        Log.d(TAG, "setAdapter: count = " + mAdapter.getCount() + ", mAdapter="
                + mAdapter + ", this=" + this);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.UNSPECIFIED && mInStrechMode
            ...
            int spaceLeftOver = minWidth
                    - ((mFixedColumnNum + 2) * requestColumnWidth); // /* M:
                                                                    // ALPS01457523
                                                                    // to
                                                                    // enlarge
                                                                    // the
                                                                    ...
                                                                    // display
                                                                    // */
            Log.d(TAG, "onMeasure 2: newHorizontalSpacing = "
                    + newHorizontalSpacing + ", spaceLeftOver = "
                    + spaceLeftOver + ",minWidth = " + minWidth
                    + ", requestColumnWidth = " + requestColumnWidth
                    + ",columnWidth = " + getColumnWidth());
            // if (spaceLeftOver > 0) ///* M: ALPS01457523 to enlarge the
            // columns 2 plus to display */
            {
            ...
            Log.d(TAG, "onMeasure: measuredWidth = " + getMeasuredWidth()
            ...
            Log.d(TAG, "onMeasure else: measuredWidth = " + getMeasuredWidth()
                    + ", measuredHeight = " + getMeasuredHeight()
                    + ", itemCount = " + getCount()
                    + ", newHorizontalSpacing = " + getHorizontalSpacing()
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (DEBUG_LAYOUT) {
            Log.d(TAG, "onLayout: mWaitingForInsertedItem = "
        ...
        if (mWaitingForInsertedItem) {
            ...
            if (sDragView != null)
                Log.d(TAG,
                        "onLayout: sDragView vis = "
                                + sDragView.getVisibility());
            mWaitingForInsertedItem = false;
        } else if (mWaitingForRemovedItem) {
            sDragView = sOriginDragView;
            Log.d(TAG, "onLayout: sDragView vis = " + sDragView.getVisibility()
        ...
        public boolean onDrag(View v, DragEvent event) {
                ...
                if (mInternalDrag) {
                    ...
                    if (getChildCount() == 0) {
                        ...
                        if ((mAdapter.getCount() > COUNT_CHILD_SHOW_IN_TREE_LINE)
                                || (mAdapter.getCount() == COUNT_CHILD_SHOW_IN_TREE_LINE)) {
                            ...
                            if (pos == -1) {
                                ...
                                Log.d(TAG,
                                        "ACTION_DRAG_ENTERED: ChildBottom = "
                                                + ChildBottom);
                                final int lastChildBottom = /*
                                                             * lastChildView.
                                                             * getBottom()
                                                             */ChildBottom;

                                final int gridViewBottom = DragSortGridView.this
                                        .getBottom();
                                final int verticalSpace = DragSortGridView.this
                                        .getVerticalSpacing();

                                Log.d(TAG, "ACTION_DRAG_ENTERED: pos = " + pos
                                        + ", lastChildBottom = "
                                        + lastChildBottom
                                        + ", gridViewBottom = "
                                        + gridViewBottom + ", verticalSpace = "
                                        + verticalSpace + ", lastChildView = "
                                        + lastChildView);

                                // If the visual grid is full of items, we need
                                // to insert the icon at the last visible
                                // position,
                                // which means the position last visible child,
                                // or
                    ...
                    Log.d(TAG, "ACTION_DRAG_ENTERED lastPosition:"
            ...
            case DragEvent.ACTION_DRAG_LOCATION:
                ...
                if (-1 != newPosition && mLastDraggingPosition != newPosition) {
                    ...
                    mDragSucceed = false;
                    Log.d(TAG, "reordering positions:" + mReorderingPositions);
                }
                break;
            ...
            case DragEvent.ACTION_DROP:
                ...
                if (mMoveInView) {
                    ...
                    pos = findInsertPosition(x, y);
                    /* for ALPS02070179 when pos is -1,put icon in tail */
                    newPosition = (pos == -1) ? (mAdapter.getCount() - 1)
                            : mReorderingPositions.getValueIndex(pos);
                    ...
                    if (DEBUG_DRAG) {
                        Log.d(TAG, "ACTION_DROP pos:" + pos + ",newPosition:"
                                + newPosition + ", oldPosition = "
                                + oldPosition + ", lastDragView = "
                                + lastDragView + ",mInsertedPos="
                                + mInsertedPos + ", this = " + this);
                    }
    ...
    private void makeDragViewVisible() {
        if (sDragView != null) {
            ...
            if (sDragView.getVisibility() == View.INVISIBLE) {
                Log.d(TAG, "Reset drag view visible: floatItem = " + floatItem
                        + ",sDragView = " + sDragView);
        ...
        if (sOriginDragView != null) {
            ...
            if (sOriginDragView.getVisibility() == View.INVISIBLE) {
                Log.d(TAG, "Reset origin drag view visible: floatItem = "
    ...
    private void clearAllAnimations() {
        ...
        Log.d(TAG, "clearAllAnimations: childrenCount = " + childrenCount
                + ", this = " + this);
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (child != null && child.getVisibility() != View.GONE) {
                ...
                if (frame.contains(x, y)) {
                    ...
                    Log.d("shan", "i = " + i);
                    Log.d("shan", "getFirstVisiblePosition() = "
                            + getFirstVisiblePosition());
                    Log.d("shan", "getFirstVisiblePosition() + i = "
    ...
    private class MoveViewAnimationListener implements
        ...
        public void onAnimationEnd(Animation animation) {
            ...
            mAnimationCount--;
            if (DEBUG_LAYOUT) {
                Log.d(TAG, "onAnimationEnd: newX = " + mNewX + ", newY = "
        ...
        public void onAnimationRepeat(Animation animation) {
            if (DEBUG_LAYOUT) {
                Log.d(TAG, "onAnimationRepeat: target = " + mTarget
        ...
        public void onAnimationStart(Animation animation) {
            mAnimationCount++;
            if (DEBUG_LAYOUT) {
                Log.d(TAG, "onAnimationStart: target = " + mTarget + ", tag = "
                        + mTarget.getTag() + ",mAnimationCount = "
                        + mAnimationCount);
            }
        }
    }
    ...
    public View getView(int reorderingPosition) {
        ...
        if (DEBUG_LAYOUT) {
            Log.d(TAG, "getView: reorderingPosition = " + reorderingPosition
    ...
    private class PositionInfo {
        ...
        public void insertItem(int index) {
            Log.d(TAG, "insertItem start: index = " + index + ",this = " + this);
            ...
            mPositions = newData;
            Log.d(TAG, "insertItem end: index = " + index + ",this = " + this);
        }
        ...
        public void removeItem(int index) {
            Log.d(TAG, "removeItem start: index = " + index + ",this = " + this);
            if (index < 0 || index >= mPositions.length) {
                Log.d(TAG, "removeItem failed: index = " + index
    ...
    private static class FloatDragShadowBuilder extends View.DragShadowBuilder {
        ...
        public void onProvideShadowMetrics(Point shadowSize,
            ...
            if (view != null) {
                ...
                Log.d(TAG, "onProvideShadowMetrics: width = " + width
        ...
        public void onDrawShadow(Canvas canvas) {
            ...
            if (view != null) {
                ...
                Log.d(TAG, "onDrawShadow: vis = " + view.getVisibility()
        ...
        private void drawZoomedIcon(Canvas canvas, Drawable drawable,
            ...
            Log.d(TAG, "drawZoomedIcon: width = " + width + ",height = "
            ...
            Bitmap newbmp = Bitmap.createBitmap(oldbmp, 0, 0, width, height,
                    matrix, true);
            Log.d(TAG, "drawZoomedIcon: newbmp width = " + newbmp.getWidth()
    ...
    private void dumpLayoutPosition() {
        ...
        for (int i = 0; i < childrenCount; i++) {
            if (this.getChildAt(i) != null) {
                ...
                Log.d(TAG, "Child[" + i + "] :" + toLayout + ",tag = "
    ...
    private static final SparseArray<String> DRAG_EVENT_ACTION =
            DEBUG_DRAG ? new SparseArray<String>() : null;
    static {
        if (DEBUG_DRAG) {
    ...
    private void scrollIfNeeded(int oldY, int newY) {
        ...
        if (DEBUG_DRAG) {
            Log.d(TAG, "scrollIfNeeded oldY=" + oldY + ", newY=" + newY
        ...
        if (newY >= 0) {
            if (newY >= oldY && newY + mTouchSlot >= getBottom()
                    && mScrollerState == SCROLL_STATE_IDLE) {
                Log.d(TAG, "scrollIfNeeded scroll down");
                ...
                mHandler.postDelayed(mScrollRunnable, DELAY_DRAG_SCROLL);
                    ...
                    && mScrollerState == SCROLL_STATE_IDLE) {
                Log.d(TAG, "scrollIfNeeded scroll up");
                mScrollerState = SCROLL_STATE_SCROLLING;
    ...
    private class ScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (DEBUG_DRAG) {
                Log.d(TAG, "ScrollRunnable run, mDragDirection="
