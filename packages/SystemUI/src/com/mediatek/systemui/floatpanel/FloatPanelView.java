...
public class FloatPanelView extends LinearLayout {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_SCREEN_OFF.equals(action)) {
                ...
                mBar.toggleFloatPanelScreenOff(); // /* M:ALPS01454734 Icon of
                                                    // multi window display on
                                                    // lock
                                                    // screen,toggleFloatPanel()
                                                    // based*/
            }
        }
    };
    ...
    protected void onFinishInflate() {
        ...
        mDetached = false;
        Log.d(TAG, "onFinishInflate: this = " + this);
        mExtentContainer = (LinearLayout) findViewById(R.id.extent_panel);
        mExtentContainer.setVisibility(View.INVISIBLE);

        mResidentScrollView = (CustomizedHorizontalScrollView) findViewById(
                R.id.resident_container);
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        // when the screen orientation change, the float button should show
        // consistent.
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (event.getKeyCode() == KeyEvent.KEYCODE_BACK) {
            if (action == KeyEvent.ACTION_DOWN) {
                Log.d(TAG, "Back key down, toggleFloatPanel...");
    ...
    private void startFloatActivity(FloatAppAdapter adapter, int position) {
        ...
        try {
            ...
            Log.d(TAG, "onItemClick: position = " + position
                    ...
                    + className);
            Log.d(TAG, "onItemClick: intent = " + intent);
            // / M:ALPS01891697,setPackage(null),avoid press two times back key
            // can exit @{
            intent.setPackage(null);
            // /@}
            mContext.startActivity(intent);
        } catch (ActivityNotFoundException e) {
            Log.d(TAG, "startFloatActivity,ActivityNotFoundException");
        }
        mBar.closeFloatPanel();
    }

    private DragSortGridView.OnReorderingListener mFloatDragSortListener =
            new DragSortGridView.OnReorderingListener() {

        @Override
        public void onReordering(int fromPosition, int toPosition) {
            Log.w(TAG, "onReordering gridView fromPosition:" + fromPosition
        ...
        public void onItemSwitched(int switchedPosition) {
            Log.d(TAG, "onItemSwitched: switchedPosition = " + switchedPosition);
            mExtentAdapter.removeItem(switchedPosition);
        }
    };

    private DragSortGridView.OnReorderingListener mUnFloatDragSortListener =
            new DragSortGridView.OnReorderingListener() {

        @Override
        public void onReordering(int fromPosition, int toPosition) {
            Log.d(TAG, "onReordering gridViewHorizontal fromPosition:"
        ...
        public void onItemSwitched(int switchedPosition) {
            Log.d(TAG, "onItemSwitched gridViewHorizontal: switchedPosition = "
                    + switchedPosition);
    ...
    public boolean isAppInstall(String packageName, String className) {
        Log.d(TAG, "isAppInstall, packageName =" + packageName + "className ="
                + className);
        if (mExtentAdapter != null) {
