...
EGLSurface EglManager::createSurface(EGLNativeWindowType window) {
    initialize();
    EGLSurface surface;
    TIME_LOG("eglCreateWindowSurface", surface = eglCreateWindowSurface(mEglDisplay, mEglConfig, window, nullptr));
...
bool EglManager::swapBuffers(EGLSurface surface, const SkRect& dirty,
    ...
    if (CC_LIKELY(Properties::swapBuffersWithDamage)) {
        ...
        TIME_LOG_BASIC("eglSwapBuffersWithDamageKHR",
            eglSwapBuffersWithDamageKHR(mEglDisplay, surface, rects, numrects));
    } else {
        TIME_LOG_BASIC("eglSwapBuffers", eglSwapBuffers(mEglDisplay, surface));
    }
#else
    TIME_LOG_BASIC("eglSwapBuffers", eglSwapBuffers(mEglDisplay, surface));
#endif
    ...
    if (err == EGL_BAD_NATIVE_WINDOW) {
        /// M: For some reason the native buffer was destroyed so not able to queue buffer.
        /// This usally happened because system_server died and then the rendering system is shut down.
        /// BufferQueue has also been abandoned. We just do not render anymore if it happened.
        ALOGE("swapBuffers encountered EGL_BAD_NATIVE_WINDOW on %p, halting rendering...", surface);
        return false;
    }
