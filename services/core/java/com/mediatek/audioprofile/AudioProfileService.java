...
public class AudioProfileService extends IAudioProfileService.Stub {
    ...
    private static final int DEFAULT_NOTIFICATION_INDEX =
            ProfileSettings.notification_stream.ordinal();
    ...
    public static final Uri SILENT_NOTIFICATION_URI =
            Uri.parse("com.mediatek.uri.silent_notificaton");
    ...
    private final List<String> mKeys =
            new ArrayList<String>(AudioProfileManager.MAX_PROFILES_COUNT);
    /** The four predefined profile keys: general, silent, meeting and outdoor */
    private final List<String> mPredefinedKeys =
            new ArrayList<String>(AudioProfileManager.PREDEFINED_PROFILES_COUNT);
    ...
    private AudioProfileSettings mProfileSettings = null;
    private static final String PROFILE_UPGRADED_KEY = "mtk_audioprofile_upgraded";
    private static final int PROFILE_UPGRADED_VALUE = 1;
     ...
     */
    private final HashMap<String, AudioProfileState> mProfileStates =
            new HashMap<String, AudioProfileState>(
    ...
    public static final boolean MTK_BESSURROUND_SUPPORT =
            SystemProperties.get("ro.mtk_bessurround_support").equals("1");
    ...
    private final AudioProfileListener mRingerModeListener = new AudioProfileListener() {
        ...
        public void onRingerModeChanged(int newRingerMode) {
            ...
            if (ringerMode == mRingerMode) {
                ...
                Log.d(TAG, "onRingerModeChanged: ringermode changed by other app, "
                        + "change profile! ringerMode = " + ringerMode);
                ...
                switch (ringerMode) {
                    ...
                    case AudioManager.RINGER_MODE_VIBRATE:
                        ...
                        if (!Scenario.MEETING.equals(activeScenario)) {
                            ...
                            setActiveProfile(mPredefinedKeys.get(
                                    Scenario.MEETING.ordinal()), false);
                        }
                        break;
                    ...
                    case AudioManager.RINGER_MODE_NORMAL:
                        ...
                        if (Scenario.SILENT.equals(activeScenario)
                                || Scenario.MEETING.equals(activeScenario)) {
                            Log.v(TAG, "RingerMode change to NORMAL, "
                                    + "change profile to last active profile!");
                            int systemVolume = mAudioManager.getStreamVolume(
            ...
            switch (activeScenario) {
                case OUTDOOR:
                    ...
                    if ((newVolume > minVolume) && (newVolume != maxVolume)
                            && mExt.shouldSyncGeneralRingtoneToOutdoor()) {
                        ...
                        Log.d(TAG, "onRingerVolumeChanged in outdoor profile, "
                                + "so change to general profile!");
                        break;
                    }
    ...
    /**
     * Vibrate setting change listener, when current vibrate
     * type is same as last one, it mean CTS is running,
     * so we should delay to persist vibrate setting to system
     * to avoid CTS fail.
     */
    private final AudioProfileListener mVibrateSettingListener = new AudioProfileListener() {
        ...
        public void onVibrateSettingChanged(int vibrateType, int vibrateSetting) {
            ...
            if (vibrateType == mLastVibrateType) {
                ...
                Log.w(TAG, "onVibrateSettingChanged: current vibrate type "
                        + " is same as last one, delay set vibrate!");
            }
    ...
    private final ContentObserver mRingtoneObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            synchronized (mActiveProfileKey) {
                ...
                String uriString = Settings.System.getStringForUser(
                        mContentResolver, Settings.System.RINGTONE,mUserId);
                Uri systemUri = (uriString == null ? null : Uri.parse(uriString));

                final boolean isPassiveChange = (
                        (activeState.mRingerStream == null && systemUri != null)
                        || (activeState.mRingerStream != null
                        && !activeState.mRingerStream.equals(systemUri)));
                Log.d(TAG, "Ringtone changed, mResetFlag = "  + mResetFlag
                        + "  isPassiveChange = " + isPassiveChange
                        + " uriString = " + uriString);
                switch (activeScenario) {
                    case GENERAL:
                    case OUTDOOR:
                        if (mExt.shouldSyncGeneralRingtoneToOutdoor()) {
                            ...
                            if (isPassiveChange && (!mResetFlag)) {
                                ...
                                persistRingtoneUriToDatabase(generalKey,
                                        AudioProfileManager.TYPE_RINGTONE,
                                        mSimId, systemUri);
                                persistRingtoneUriToDatabase(outdoorKey,
                                        AudioProfileManager.TYPE_RINGTONE,
                                        mSimId, systemUri);
                                Log.d(TAG, "Ringtone changed by other app in non-silent "
                                        + "profile, synchronize to active profile: "
                                        + "new uri = " + systemUri);
                            } else {
                    ...
                    case CUSTOM:
                        ...
                        if (isPassiveChange && (!mResetFlag)) {
                            ...
                            persistRingtoneUriToDatabase(mActiveProfileKey,
                                    AudioProfileManager.TYPE_RINGTONE,
                                    mSimId, systemUri);
                    ...
                    default:
                        ...
                        Log.d(TAG, "Ringtone changed in silent profile, "
                                + "sync to system if switch to last active profile.");
                        break;
                }
            }
        }
    ...
    private final ContentObserver mNotificationObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            synchronized (mActiveProfileKey) {
                ...
                AudioProfileState activeState = getProfileState(mActiveProfileKey);
                String uriString = Settings.System.getStringForUser(
                        mContentResolver, Settings.System.NOTIFICATION_SOUND,mUserId);
                Uri systemUri = (uriString == null ?
                        SILENT_NOTIFICATION_URI : Uri.parse(uriString));

                final boolean isPassiveChange = (
                        (activeState.mNotificationStream == null && systemUri != null)
                        || (activeState.mNotificationStream != null
                        && !activeState.mNotificationStream.equals(systemUri)));
                switch (activeScenario) {
                    case GENERAL:
                    case OUTDOOR:
                        if (mExt.shouldSyncGeneralRingtoneToOutdoor()) {
                    ...
                    default:
                        ...
                        Log.d(TAG, "Notification changed in silent profile, "
                                + "sync to system if switch to last active profile");
                        break;
                }
            }
     ...
     * media scanner finished, because MediaScanner start after AudioProfileService.
     */
    private final BroadcastReceiver mUpgradeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_MEDIA_SCANNER_FINISHED.equals(action)) {
                // Get default ringtone if need after scan finish
                // because maybe the default ringtone get before is null.
                if (mShouldGetDefaultRingtoneAfterScanFinish) {
                    ...
                    // If after scan finish the default ringtone uri is still null,
                    // we need read defaultringtone again
                    // when MediaScanner scan finish next time.
                ...
                for (String profileKey : allKeys) {
                    ...
                    if (SystemProperties.get("ro.mtk_multisim_ringtone").equals("1")) {
                        ...
                        for (int i = 0; i < numSlots; i++) {
                            ...
                            updateRintoneUriWithType(profileKey,
                                    AudioProfileManager.TYPE_RINGTONE,
                                    volumePath, simId);
                    ...
                    updateRintoneUriWithType(profileKey, AudioProfileManager.TYPE_RINGTONE,
                            volumePath);
                    // Update video call uri
                    updateRintoneUriWithType(profileKey, AudioProfileManager.TYPE_VIDEO_CALL,
                            volumePath);
                    // Update notification uri
                    updateRintoneUriWithType(profileKey, AudioProfileManager.TYPE_NOTIFICATION,
                            volumePath);
                    // Update sip call uri
                    updateRintoneUriWithType(profileKey, AudioProfileManager.TYPE_SIP_CALL,
                            volumePath);
    ...
    private final BroadcastReceiver mBootCompleteReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_BOOT_COMPLETED.equals(action)) {
                ...
                persistValues(false);
                // When current ringermode is RINGER_MODE_NORMAL and the ringer volume is 0,
                // AudioService will restore volume to be default value,
                // so AudioProfileService need to reset the volume to be 0 when boot complete.
                Scenario activeScenario = AudioProfileManager.getScenario(mActiveProfileKey);
                if (mRingerMode == AudioManager.RINGER_MODE_NORMAL
                        && getStreamVolume(
                        mActiveProfileKey, AudioProfileManager.STREAM_RING) == 0) {
                    mAudioManager.setAudioProfileStreamVolume(
                            AudioProfileManager.STREAM_RING, 0, 0);
                    mAudioManager.setAudioProfileStreamVolume(
                            AudioProfileManager.STREAM_NOTIFICATION, 0, 0);
                    Log.d(TAG, "Persist system volume to be 0 if ringermode is normal "
                            + "and volume is 0 when boot complete!");
                }
            }
    ...
    private void updateRintoneUriWithType(String profileKey, int ringtoneType,
            String[] volumePath, long simId) {
        Log.v(TAG, "updateRintoneUriWithType  " + profileKey + " with ringtoneType  "
                + ringtoneType + " simId = " + simId);
        // Update ringtone uri with ringtone type

        String dataKey = AudioProfileManager.getDataKey(
                AudioProfileManager.getStreamUriKey(profileKey, ringtoneType, simId));
        Uri newUri = updateRintoneUri(dataKey, profileKey, ringtoneType, volumePath);
        Log.v(TAG, "updateRintoneUriWithType   uri = " + newUri
                + "  with ringtoneType  " + ringtoneType);
    ...
    private void updateRintoneUriWithType(String profileKey, int ringtoneType,
            String[] volumePath) {
        Log.v(TAG, "updateRintoneUriWithType  " + profileKey
                + " with ringtoneType  " + ringtoneType);
        // Update ringtone uri with ringtone type
    ...
    private Uri updateRintoneUri(String dataKey, String profileKey, int streamType,
            String[] volumePath) {

        Log.v(TAG, "updateRintoneUri :  " + profileKey + " with ringtoneType  "
                + streamType +  "  -  " + volumePath);
        AudioProfileState profileState = getProfileState(profileKey);
        Uri oldUri = null;
        ...
        if ((null != oldUri) && (oldUri.toString().startsWith(
                MediaStore.Audio.Media.EXTERNAL_CONTENT_URI.toString()))) {
            Uri newUri = null;
            String uriData = Settings.System.getStringForUser(mContentResolver, dataKey,mUserId);
            if (null == uriData) {
                return null;
            }
            ...
            for (int i = 0; i < length; i++) {
                ...
                try {
                    if (null != cursor && cursor.moveToFirst()) {
                        ...
                        Log.v(TAG, "Update ringtone uri for " + dataKey
                                + " with new uri: " + newUri);
                        return newUri;
                    }
                } finally {
    ...
    public AudioProfileService(Context context) {
        ...
        mContext = context;
        mProfileSettings = AudioProfileSettings.getInstance(mUserId);
        mContentResolver = mContext.getContentResolver();
        mAudioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
        mStorageManager = (StorageManager) context.getSystemService(Context.STORAGE_SERVICE);
        AudioProfileManager audioProfileManager =
        ...
        String activeProfileKey =
                mProfileSettings.get(AudioProfileManager.KEY_ACTIVE_PROFILE, null);
        mActiveProfileKey = (activeProfileKey == null ?
                mPredefinedKeys.get(Scenario.GENERAL.ordinal()) : activeProfileKey);
        String lastActiveProfileKey =
                mProfileSettings.get(AudioProfileManager.LAST_ACTIVE_PROFILE, null);
        mLastActiveProfileKey =
                (lastActiveProfileKey == null ?
                mPredefinedKeys.get(Scenario.GENERAL.ordinal()) : lastActiveProfileKey);
        mIsLastCustomActiveProfileDeleted =
                Boolean.valueOf(mProfileSettings
                .get(AudioProfileManager.LAST_ACTIVE_CUSTOM_DELETED, "false"));
        ...
        mAudioManager.listenRingerModeAndVolume(mRingerModeListener,
                AudioProfileListener.LISTEN_RINGERMODE_CHANGED);
        mAudioManager.listenRingerModeAndVolume(mRingerVolumeListener,
        ...
        mContentResolver.registerContentObserver(
                Settings.System.getUriFor(Settings.System.RINGTONE),
                                          false,
                                          mRingtoneObserver);
        mContentResolver.registerContentObserver(
                Settings.System.getUriFor(Settings.System.NOTIFICATION_SOUND), false,
                mNotificationObserver);

        // Initial mShouldSyncToSystem
        readShouldSyncToSystem();

        // Register media scan finish receiver for When first boot a phone,
        // AudioProfileService are initial before media scanner,
        // so we should update the default ringtone
        //when media scanner finish initial database.
        ...
        mContext.registerReceiverAsUser(mUserSwitchReceiver,
                                        UserHandle.CURRENT,
                                        userSwitchFilter,
                                        null,
                                        null);
        ...
        } else if (null == RingtoneManager.getActualDefaultRingtoneUri(mContext,
                AudioProfileManager.TYPE_RINGTONE)) {
        ...
        IAudioProfileExtension tmpExt = MPlugin.createInstance(
                IAudioProfileExtension.class.getName(), mContext);
        if (tmpExt != null) {
            mExt = tmpExt;
        } else {
    ...
    private void reInit(){
        ...
        // Initial active profile key and last active profile key, if not exist,
        // use general as defalut.
        String oldActiveProfileKey = mActiveProfileKey;
        String activeProfileKey =
                mProfileSettings.get(AudioProfileManager.KEY_ACTIVE_PROFILE, null);
        mActiveProfileKey = (activeProfileKey == null ?
                mPredefinedKeys.get(Scenario.GENERAL.ordinal()) : activeProfileKey);
        ...
        if (mActiveProfileKey.equals(mPredefinedKeys.get(Scenario.OUTDOOR.ordinal()))
                || mActiveProfileKey.equals(
                mPredefinedKeys.get(Scenario.GENERAL.ordinal()))) {
            Log.i(TAG, "Sync general to outdoor.");
            AudioProfileState generalState = mProfileStates
                    .get(mPredefinedKeys.get(Scenario.GENERAL.ordinal()));
            AudioProfileState outdoorState = mProfileStates
                    .get(mPredefinedKeys.get(Scenario.OUTDOOR.ordinal()));
        ...
        if (mActiveProfileKey.equals(mPredefinedKeys.get(Scenario.SILENT.ordinal()))) {
            persistValues(false);
            mRingerMode = AudioManager.RINGER_MODE_SILENT;
        } else if(mActiveProfileKey.equals(mPredefinedKeys.get(Scenario.MEETING.ordinal()))){
            persistValues(false);
        ...
        // The mActiveProfileKey maybe changed after MOTA/OTA upgrade
        if(oldActiveProfileKey != mActiveProfileKey){
    ...
    public void setActiveProfile(String profileKey, boolean shouldSetRingerMode) {
        ...
        Log.d(TAG, "setActiveProfile>>>: oldProfileKey = " + oldProfileKey
        ...
        synchronized (mLock) {
            ...
            IAudioProfileExtension.IActiveProfileChangeInfo apcInfo =
                    mExt.getActiveProfileChangeInfo(shouldSetRingerMode, oldProfileKey,
                            newProfileKey, mIsLastCustomActiveProfileDeleted);
            ...
            if (apcInfo == null) {
                ...
                switch (newScenario) {
                    case SILENT:
                        // Set RingerMode:
                        setRingerModeMatchProfile(shouldSetRingerMode,
                    ...
                    default:
                        ...
                        if (getProfileState(newProfileKey).mRingerVolume == 0) {
                            int volume = mAudioManager
                                    .getStreamVolume(AudioProfileManager.STREAM_RING);
                            syncRingerVolumeToProfile(newProfileKey, volume);
                            Log.d(TAG, "setActiveProfile: profile volume is 0, change to "
                                    + volume);
                        }
                        // Persisted settings to override system
                        if ((Scenario.SILENT.equals(oldScenario)
                                || Scenario.MEETING.equals(oldScenario))
                            ...
                            overrideSystem = mIsLastCustomActiveProfileDeleted
                                    || mShouldOverrideSystem;
                ...
                if (ringerModeToUpdate != IAudioProfileExtension
                            .IActiveProfileChangeInfo.RINGER_MODE_UPDATE_NONE) {
                    mRingerMode = ringerModeToUpdate;
                    mAudioManager.setRingerModeInternal(ringerModeToUpdate);
                    Log.d(TAG, "CMCC: setActiveProfile: RingerMode now set "
                            + ringerModeToUpdate);
                }
    ...
    private void setOldProfileSettings(Scenario oldScenario, String oldProfileKey) {
        ...
        if (Scenario.GENERAL.equals(oldScenario) || Scenario.CUSTOM.equals(oldScenario)
                || (Scenario.OUTDOOR.equals(oldScenario)
                && !mExt.shouldSyncGeneralRingtoneToOutdoor())) {
    ...
    private void setRingerModeMatchProfile(boolean shouldSetRingerMode, int expectRingerMode) {
        ...
        Log.d(TAG, "setRingerModeMatchProfile: actual = " + actualRingerMode
                + ", expect = " + expectRingerMode);
    }
    ...
    private boolean setActiveKey(String profileKey) {
        ...
        boolean succeed = mProfileSettings.put(AudioProfileManager.KEY_ACTIVE_PROFILE, profileKey);
    ...
    private boolean setLastActiveKey(String profileKey) {
        final long token = Binder.clearCallingIdentity();
        boolean succeed = mProfileSettings.put(AudioProfileManager.LAST_ACTIVE_PROFILE, profileKey);
    ...
    private boolean setLastCustomActiveDeleted(boolean deleted) {
        boolean succeed = mProfileSettings.put(AudioProfileManager.LAST_ACTIVE_CUSTOM_DELETED,
    ...
    private void notifyAudioProfileChanged() {
        ...
        synchronized (mRecords) {
            ...
            Log.d(TAG, "notifyAudioProfileChanged: New profile = " + mActiveProfileKey
    ...
    public String addProfile() {
        ...
        AudioProfileState newProfileState = new AudioProfileState.Builder(newKey)
            .ringtone(mDefaultRingtone.get(DEFAULT_RINGER_INDEX),
                    ...
                    mDefaultRingtone.get(DEFAULT_VIDEOCALL_INDEX),
                    mDefaultRingtone.get(DEFAULT_SIPCALL_INDEX))
            .volume(defaultState.mRingerVolume, defaultState.mNotificationVolume,
                    defaultState.mAlarmVolume)
        ...
        boolean succeed = mProfileSettings.put(name,newKey);
        // Save the creation date for sorting
        mProfileSettings.put(AudioProfileManager.getProfileCreateDateKey(newKey),
    ...
    public boolean deleteProfile(String profileKey) {
        ...
        boolean removed = mProfileSettings.remove(keyList);
        if(removed){
            synchronized (mLock) {
    ...
    private String genCustomKey() {
        int maxCustom = AudioProfileManager.MAX_PROFILES_COUNT
                - AudioProfileManager.PREDEFINED_PROFILES_COUNT;
    ...
    public List<String> getAllProfileKeys() {
        synchronized (mLock) {
            ...
            Log.d(TAG, "getAllProfileKeys: UserManager.supportsMultipleUsers()"
                    + UserManager.supportsMultipleUsers());
            Log.d(TAG, "getAllProfileKeys: mUserId = " + mUserId
    ...
    private void persistValues(boolean overrideSystem) {
        ...
        if (null == activeProfileState) {
            Log.e(TAG, "persistValues error with no " + mActiveProfileKey + " in "
                    + mProfileStates);
        } else {
            Log.d(TAG, "persistValues: override = " + overrideSystem + ", state = "
                    + activeProfileState.toString());
        }
    ...
    private void restoreToDefaultValues(String profileKey) {
        ...
        List<String> keyList = AudioProfileManager.getAllKeys(profileKey);
        mProfileSettings.remove(keyList);
        Log.d(TAG, "restoreToDefaultValues: profileKey = " + profileKey);
    }

    /**
     * sync the three type volumes to system that has been changed
     * in last active profile's settings
    ...
    public Uri getRingtoneUri(String profileKey, int type) {
        ...
        Log.d(TAG, "getRingtoneUri: profileKey = " + profileKey + ", type = "
                + type + ", uri = " + uri);
        return uri;
    }
    ...
    public Uri getRingtoneUriWithSIM(String profileKey, int type, long simId) {
        ...
        Log.d(TAG, "getRingtoneUriWithSIM: profileKey = " + profileKey + ", type = " + type
                + ", uri = " + uri + ", simId = " + simId);
        return uri;
    ...
    public int getStreamVolume(String profileKey, int streamType) {
        ...
        Log.d(TAG, "getStreamVolume: profileKey = " + profileKey + ", streamType = "
                + streamType + ", volume = " + validVolume);
        return validVolume;
    }
    ...
    /**
     * Gets whether the phone should vibrate when pressing soft keys and
    ...
    public boolean isVibrateOnTouchEnabled(String profileKey) {
        ...
        Log.d(TAG, "isVibrateOnTouchEnabled: profileKey = " + profileKey + ", enabled = "
                + enabled);
        return enabled;
    }
    ...
    public AudioProfileState getProfileState(String profileKey, long simId) {
        synchronized (mLock) {
            ...
            if (profileState == null) {
                ...
                Log.w(TAG, "getProfileState of " + profileKey
                        + "is null, so create new one instead!");
    ...
    public List<String> getProfileStateString(String profileKey) {
        ...
        state.add(ProfileSettings.ringer_volume.ordinal(),
                String.valueOf(profileState.mRingerVolume));
        state.add(ProfileSettings.notification_volume.ordinal(),
                String.valueOf(profileState.mNotificationVolume));
        state.add(ProfileSettings.alarm_volume.ordinal(),
                String.valueOf(profileState.mAlarmVolume));

        state.add(ProfileSettings.vibration_enabled.ordinal(),
                String.valueOf(profileState.mVibrationEnabled));
        state.add(ProfileSettings.dtmftone_enabled.ordinal(),
                String.valueOf(profileState.mDtmfToneEnabled));
        state.add(ProfileSettings.soundeffect_enbled.ordinal(),
                String.valueOf(profileState.mSoundEffectEnbled));
        state.add(ProfileSettings.lockscreensound_enabled.ordinal(),
                String.valueOf(profileState.mLockScreenSoundEnabled));
        state.add(ProfileSettings.hapticfeedback_enabled.ordinal(),
                String.valueOf(profileState.mHapticFeedbackEnabled));
    ...
    public String getProfileName(String profileKey) {
        ...
        Log.d(TAG, "getProfileName: profileKey = " + profileKey
    ...
    public void setRingtoneUri(String profileKey, int type, long simId, Uri ringtoneUri) {
        ...
        Log.d(TAG, "setRingtoneUri: profileKey = " + profileKey + ", type = "
                + type + ", uri = " + newRingtoneUri);
    }

    private boolean isValidRingtoneType(int type) {
        return (AudioProfileManager.TYPE_RINGTONE == type)
                || (AudioProfileManager.TYPE_NOTIFICATION == type)
                || (AudioProfileManager.TYPE_VIDEO_CALL == type)
                || (AudioProfileManager.TYPE_SIP_CALL == type);
    }

    private boolean isRingtoneUriChanged(AudioProfileState profileState, int type,
            Uri newRingtoneUri) {
        switch (type) {
            case AudioProfileManager.TYPE_RINGTONE:
    ...
    public void setStreamVolume(String profileKey, int streamType, int index) {
        synchronized (mLock) {
            ...
            switch (streamType) {
                case AudioProfileManager.STREAM_RING:
                    if (profileState.mRingerVolume != validIndex) {
                        // If the profile's volume is different from given volume, set the volume
                        // to be profile volume into database and save this change to profile
                        // states hash map
                        persistStreamVolumeToDatabase(profileKey, streamType, validIndex);
                        profileState.mRingerVolume = validIndex;

                        // If the profile is last active profile, we should save user change and
                        // sync to system when the profile was set to active profile.
                        if (profileKey.equals(mLastActiveProfileKey)) {
                ...
                case AudioProfileManager.STREAM_NOTIFICATION:
                    if (profileState.mNotificationVolume != validIndex) {
                        // If the profile's volume is different from given volume, set the volume
                        // to the profile volume into database and save this change to profile
                        // states hash map
                        persistStreamVolumeToDatabase(profileKey, streamType, validIndex);
                        ...
                        // sync to system when the profile was set to active profile.
                        if (profileKey.equals(mLastActiveProfileKey)) {
                            mShouldSyncToSystem.set(
                                    ProfileSettings.notification_volume.ordinal(), true);
                        }
                    }
                    break;
            ...
            Log.d(TAG, "setStreamVolume: profileKey = " + profileKey + ", streamType = "
                    + streamType + ", volume = " + validIndex);
        }
    }
    ...
    /**
     * Sets whether the phone should vibrate for incoming calls,
     * and need check whether we need set ringermode to match
     * vibration change. We need not set ringermode
     * when this method is called trigger by ringermode changed.
     *
            ...
            if (profileState.mVibrationEnabled != enabled) {
                ...
                if (Scenario.SILENT.equals(AudioProfileManager.getScenario(profileKey))) {
                    Settings.System.putIntForUser(mContentResolver,
                        Settings.System.VIBRATE_IN_SILENT, enabled ? 1 : 0, mUserId);
    ...
    public void setDtmfToneEnabled(String profileKey, boolean enabled) {
        synchronized (mLock) {
            ...
            if (profileState.mDtmfToneEnabled != enabled) {
                ...
                Log.d(TAG, "setDtmfToneEnabled: profileKey = " + profileKey + ", enabled = "
                    + enabled);
            }
        }
    }
    ...
    public void setSoundEffectEnabled(String profileKey, boolean enabled) {
        synchronized (mLock) {
            ...
            if (profileState.mSoundEffectEnbled != enabled) {
                ...
                Log.d(TAG, "setSoundEffectEnabled: profileKey = " + profileKey + ", enabled = "
                    + enabled);
            }
        }
    }
    ...
    public void setLockScreenEnabled(String profileKey, boolean enabled) {
        synchronized (mLock) {
            ...
            if (profileState.mLockScreenSoundEnabled != enabled) {
                ...
                Log.d(TAG, "setLockScreenEnabled: profileKey = " + profileKey + ", enabled = "
                    + enabled);
            }
        }
    }
    ...
    /**
     * Sets whether the phone should vibrate when pressing soft keys and
     * on certain UI interactions.
     *
     * @param profileKey
     *            The profile key given to set vibration enabled.
    ...
    public void setVibrateOnTouchEnabled(String profileKey, boolean enabled) {
        synchronized (mLock) {
            ...
            if (profileState.mHapticFeedbackEnabled != enabled) {
                ...
                Log.d(TAG, "setVibrateOnTouchEnabled: profileKey = " + profileKey + ", enabled = "
                        + enabled);
            }
        }
    }
    ...
    private void persistRingtoneUriToDatabase(String profileKey, int type, long simId, Uri uri) {
        ...
        switch (type) {
            case AudioProfileManager.TYPE_RINGTONE:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_VOICECALL_RINGTONE_TO_DATABASE, -1, -1, name);
                break;

            case AudioProfileManager.TYPE_NOTIFICATION:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_NOTIFICATION_RINGTONE_TO_DATABASE, -1, -1, name);
                break;

            case AudioProfileManager.TYPE_VIDEO_CALL:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_VIDEOCALL_RINGTONE_TO_DATABASE, -1, -1, name);
                break;

            case AudioProfileManager.TYPE_SIP_CALL:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_SIPCALL_RINGTONE_TO_DATABASE, -1, -1, name);
                break;
    ...
    private void persistStreamVolumeToDatabase(String profileKey, int streamType, int value) {
        ...
        switch (streamType) {
            case AudioProfileManager.STREAM_RING:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_RINGER_VOLUME_TO_DATABASE, value, -1, name);
                break;

            case AudioProfileManager.STREAM_NOTIFICATION:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_NOTIFICATION_VOLUME_TO_DATABASE, value, -1, name);
                break;

            case AudioProfileManager.STREAM_ALARM:
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_ALARM_VOLUME_TO_DATABASE, value, -1, name);
                break;
    ...
    private void persistVibrationToDatabase(String profileKey, boolean enabled) {
        ...
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_VIBRATION_TO_DATABASE, -1, -1, name);
        msg.setData(bundle);
        msg.sendToTarget();
    }
    ...
    private void persistDtmfToneToDatabase(String profileKey, boolean enabled) {
        ...
        Log.d(TAG, "persistDtmfToneToDatabase() name = " +name);
        bundle.putString(name, String.valueOf(enabled));
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_DTMF_TONE_TO_DATABASE, -1, -1, name);
        msg.setData(bundle);
        msg.sendToTarget();
    }
    ...
    private void persistSoundEffectToDatabase(String profileKey, boolean enabled) {
        ...
        bundle.putString(name, String.valueOf(enabled));
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_SOUND_EFFECT_TO_DATABASE, -1, -1, name);
        msg.setData(bundle);
        msg.sendToTarget();
    }
    ...
    private void persistLockScreenToDatabase(String profileKey, boolean enabled) {
        ...
        bundle.putString(name, String.valueOf(enabled));
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_LOCKSCREEN_SOUND_TO_DATABASE, -1, -1, name);
        msg.setData(bundle);
        msg.sendToTarget();
    }
    ...
    private void persistHapticFeedbackToDatabase(String profileKey, boolean enabled) {
        ...
        bundle.putString(name, String.valueOf(enabled));
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_HAPTIC_FEEDBACK_TO_DATABASE, -1, -1, name);
        msg.setData(bundle);
        msg.sendToTarget();
    }
    ...
    private void persistProfileNameToDatabase(String profileKey, String profileName) {
        ...
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_PROFILE_NAME_TO_DATABASE, -1, -1, name);
        msg.setData(bundle);
        msg.sendToTarget();
    }
    ...
    private void persistRingtoneUriToSystem(int type) {
        ...
        switch (type) {
            case AudioProfileManager.TYPE_RINGTONE:
                ...
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_VOICECALL_RINGTONE_TO_SYSTEM, -1, -1, name);
                break;
            ...
            case AudioProfileManager.TYPE_NOTIFICATION:
                ...
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_NOTIFICATION_RINGTONE_TO_SYSTEM, -1, -1, name);
                break;
            ...
            case AudioProfileManager.TYPE_VIDEO_CALL:
                ...
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_VIDEOCALL_RINGTONE_TO_SYSTEM, -1, -1, name);
                break;
            ...
            case AudioProfileManager.TYPE_SIP_CALL:
                ...
                msg = mAudioProfileHandler.obtainMessage(
                        MSG_PERSIST_SIPCALL_RINGTONE_TO_SYSTEM, -1, -1, name);
                break;
    ...
    private void persistStreamVolumeToSystem(int streamType) {
        ...
        try {
            ...
            Log.d(TAG, "persistStreamVolumeToSystem: streamType = " + streamType + ", volume = "
                    + volume);
        } finally {
    ...
    private void persistVibrationToSystem() {
        ...
        if (vibratinRinger != vibratinNotification) {
            ...
            mAudioProfileHandler.sendEmptyMessageDelayed(
                    MSG_DELAY_SET_VIBRATE_AVOID_CTS_FAIL, DELAY_TIME_AVOID_CTS_FAIL);
            mDelaySetVibrate = true;
            return;
        }
        if (mDelaySetVibrate) {
            Log.d(TAG, "persistVibrationToSystem: CTS test running, "
                    + "delay 20 sec to set vibration!");
            return;
        }
    ...
    private void persistDtmfToneToSystem() {
        ...
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_DTMF_TONE_TO_SYSTEM, enabled, -1, name);
        msg.sendToTarget();
    }
    ...
    private void persistSoundEffectToSystem() {
        ...
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_SOUND_EFFECT_TO_SYSTEM, enabled, -1, name);
        msg.sendToTarget();
    }
    ...
    private void persistLockScreenToSystem() {
        ...
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_LOCKSCREEN_SOUND_TO_SYSTEM, enabled, -1, name);
        msg.sendToTarget();
    }
    ...
    private void persistHapticFeedbackToSystem() {
        ...
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_HAPTIC_FEEDBACK_TO_SYSTEM, enabled, -1, name);
        msg.sendToTarget();
    }
    ...
    private void persistValuesToSystemByBatch(ArrayList<ContentProviderOperation> ops) {
        Message msg = mAudioProfileHandler.obtainMessage(
                MSG_PERSIST_VALUES_TO_SYSTEM_BY_BATCH, ops);
        msg.sendToTarget();
    }
    ...
    private void readAllProfileKeys() {
        ...
        Log.d(TAG, "readProfileKeys: started");
        mKeys.clear();
        mKeys.addAll(mPredefinedKeys);
        int maxCustom = AudioProfileManager.MAX_PROFILES_COUNT
                        - AudioProfileManager.PREDEFINED_PROFILES_COUNT;
        String customPrefix = AudioProfileManager.getProfileKey(Scenario.CUSTOM);
        List<String> customeKey = mProfileSettings.getAllKeys();
        Hashtable<Long,String> customizeProfiles = new Hashtable<Long,String>();
        for(String key : customeKey){
            if(key.startsWith(customPrefix) && key.endsWith(AudioProfileManager.SUFFIX_KEY)){
                String profile =mProfileSettings.get(key,"");
                Log.d(TAG, "readProfileKeys() profile key = " + key + ", profile = " + profile);
                if(profile == ""){
                    continue;
                }
                String strDateKey = AudioProfileManager.getProfileCreateDateKey(profile);
                String strDateVal = mProfileSettings.get(strDateKey,
                                                     String.valueOf(System.currentTimeMillis()));
                customizeProfiles.put(Long.parseLong(strDateVal),profile);
            }
        }

        //sort
        List<Long> list = new ArrayList<Long>();
        for(Long key :customizeProfiles.keySet()){
            list.add(key);
        }
        ...
        for(int i=0;i<list.size();i++){
           mKeys.add(customizeProfiles.get(list.get(i)));
        }
    ...
    private void readPredefinedProfileKeys() {
        mPredefinedKeys.clear();
        mPredefinedKeys.add(Scenario.GENERAL.ordinal(),
                AudioProfileManager.getProfileKey(Scenario.GENERAL));
        mPredefinedKeys.add(Scenario.SILENT.ordinal(),
                AudioProfileManager.getProfileKey(Scenario.SILENT));
        mPredefinedKeys.add(Scenario.MEETING.ordinal(),
                AudioProfileManager.getProfileKey(Scenario.MEETING));
        mPredefinedKeys.add(Scenario.OUTDOOR.ordinal(),
                AudioProfileManager.getProfileKey(Scenario.OUTDOOR));
    ...
    private void readPersistedSettings(String profileKey, long simId) {
        ...
        Log.e(TAG, "readPersistedSettings()  mUserId = " + mUserId);
        ...
        List<String> keys = mProfileSettings.getAllKeys();
        List<String> values = mProfileSettings.getAllValues();
        ...
        if(keysSize == valuesSize){
            for(int i=0;i<keysSize;i++){
                ...
                if(key != null
                  && key != ""
                  && key.contains(profileKey)
                  && value != null
                  && value != ""){
                    Log.e(TAG, "readPersistedSettings() key = " + key + ", value = " + value);
        ...
        name = AudioProfileManager.getStreamUriKey(profileKey,
                AudioProfileManager.TYPE_VIDEO_CALL, simId);
        Uri videoCallUri = getPersistedValue(name, initValues,
                mDefaultRingtone.get(DEFAULT_VIDEOCALL_INDEX));

        name = AudioProfileManager.getStreamUriKey(profileKey,
                AudioProfileManager.TYPE_SIP_CALL, simId);
        ...
        name = AudioProfileManager.getStreamVolumeKey(profileKey,
                AudioProfileManager.STREAM_NOTIFICATION);
        int notificationVolume = getPersistedValue(name, initValues,
                defaultState.mNotificationVolume);
        ...
        boolean lockScreensound = getPersistedValue(name, initValues,
                defaultState.mLockScreenSoundEnabled);

        name = AudioProfileManager.getHapticKey(profileKey);
        boolean hapticFeedback = getPersistedValue(name, initValues,
                defaultState.mHapticFeedbackEnabled);
    ...
    private void readDefaultRingtones() {
        ...
        uriString = Settings.System.getStringForUser(mContentResolver,
                AudioProfileManager.KEY_DEFAULT_NOTIFICATION,mUserId);
        ...
        uriString = Settings.System.getStringForUser(mContentResolver,
                AudioProfileManager.KEY_DEFAULT_VIDEO_CALL,mUserId);
        uri = (uriString == null ? null : Uri.parse(uriString));
    ...
    private void readShouldSyncToSystem() {
        ...
        if (Scenario.SILENT.equals(activeScenario) || (Scenario.MEETING.equals(activeScenario))) {
            ...
            // Ringtone
            Uri systemUri = RingtoneManager.getActualDefaultRingtoneUri(mContext,
                    AudioProfileManager.TYPE_RINGTONE);
    ...
    private void checkDefaultProfiles() {
        ...
        if (mExt.shouldSyncGeneralRingtoneToOutdoor()) {
            ...
            AudioProfileState generalState = mProfileStates.get(
                    mPredefinedKeys.get(Scenario.GENERAL.ordinal()));
            AudioProfileState outdoorState = mProfileStates.get(
                    mPredefinedKeys.get(Scenario.OUTDOOR.ordinal()));
            // Sync three type ringtones from general to outdoor
            outdoorState.mRingerStream = generalState.mRingerStream;
            outdoorState.mNotificationStream = generalState.mNotificationStream;
            outdoorState.mVideoCallStream = generalState.mVideoCallStream;
        ...
        for (String profileKey : mKeys) {
            mProfileStates.get(profileKey).mNotificationVolume =
                    mProfileStates.get(profileKey).mRingerVolume;
        }
        Log.d(TAG, "checkDefaultProfiles<<<");
    }
    ...
    public void syncRingerVolumeToProfile(String profileKey, int volume) {
        if (getProfileState(profileKey).mRingerVolume != volume) {
            ...
            persistStreamVolumeToDatabase(profileKey, AudioProfileManager.STREAM_NOTIFICATION,
                    volume);
            getProfileState(profileKey).mRingerVolume = volume;
            getProfileState(profileKey).mNotificationVolume = volume;
            Log.d(TAG, "syncRingerVolumeToProfile: profileKey = " + profileKey + ", volume = "
                    + volume);
        }
    }
     ...
     */
    private Uri getPersistedValue(String name, HashMap<String, String> initValues,
            Uri defaultUri) {
        if (name != null) {
     ...
     * @return The persisted value if the setting exists in database, otherwise the defaultValue.
     */
    private int getPersistedValue(String name, HashMap<String, String> initValues,
            int defaultValue) {
        if (name != null) {
     ...
     * @return The persisted value if the setting exists in database, otherwise the defaultValue.
     */
    private boolean getPersistedValue(String name, HashMap<String, String> initValues,
        ...
        synchronized (mRecords) {
            ...
            if (event == AudioProfileListener.LISTEN_PROFILE_CHANGE) {
                try {
                    ...
                    Log.e(TAG, "Dead object in listenAudioProfie, remove listener's callback!"
                            + e);
                }
            }

            Log.d(TAG, "listenAudioProfie with event = " + event + " sucessed, record.mBinder = "
    ...
    public void setUserId(int userId) {
        Log.d(TAG,"setUserId() lastUserId = " + mUserId + " ,mUserId = " + userId);
        if(mUserId != userId){
            mUserId = userId;
            reInit();
        }
    ...
    class AudioProfileHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            Log.d(TAG, "handleMessage what = " + msg.what + ", name = " + name + ", value = "
                    + valueInt + " or " + valueSting);
            switch (msg.what) {
                case MSG_PERSIST_VOICECALL_RINGTONE_TO_SYSTEM:
                    RingtoneManager.setActualDefaultRingtoneUri(
                            mContext, AudioProfileManager.TYPE_RINGTONE,
                            (valueSting == null ? null : Uri.parse(valueSting)));
                    break;

                case MSG_PERSIST_NOTIFICATION_RINGTONE_TO_SYSTEM:
                    RingtoneManager.setActualDefaultRingtoneUri(
                            mContext, AudioProfileManager.TYPE_NOTIFICATION,
                            (valueSting == null ? null : Uri.parse(valueSting)));
                    break;

                case MSG_PERSIST_VIDEOCALL_RINGTONE_TO_SYSTEM:
                    RingtoneManager.setActualDefaultRingtoneUri(
                            mContext, AudioProfileManager.TYPE_VIDEO_CALL,
                            (valueSting == null ? null : Uri.parse(valueSting)));
                    break;
                ...
                case MSG_PERSIST_HAPTIC_FEEDBACK_TO_SYSTEM:
                    // These info need to write to settings provider
                    Settings.System.putIntForUser(mContentResolver,
                                                  name,
                                                  msg.arg1,
                                                  mUserId);
                ...
                case MSG_PERSIST_ALARM_VOLUME_TO_DATABASE:
                    mProfileSettings.put(name,String.valueOf(valueInt));
                    break;
    ...
    private void initBesSurroundStatus() {
        String bessuroundState = mProfileSettings.get(
                                      AudioProfileManager.KEY_BESSUROUND_STATE, "-1");
        int BesSurroundCode = Integer.valueOf(bessuroundState);
        String bessuroundMode = mProfileSettings.get(
                                      AudioProfileManager.KEY_BESSUROUND_MODE, "-1");
        int BesSurroundMode = Integer.valueOf(bessuroundMode);
        if (BesSurroundCode == -1) {
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_STATE,
                                 String.valueOf(AudioProfileManager.KEY_SURROUND_OFF));
        }
        ...
        if (BesSurroundMode == -1) {
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_MODE,
                                 String.valueOf(AudioProfileManager.KEY_MOVIE_MODE_CODE));
        }
    ...
    public int setBesSurroundState(boolean isEnableBes) {
        ...
        mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_STATE, String.valueOf(stateCode));
    ...
    public int setBesSurroundMode(int besSurroundMode) {
        ...
        mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_MODE,
                              String.valueOf(besSurroundMode));
    ...
    /**
     * This class defined to save all the audio profile related information
     *
     * File location on device: /data/system/audio_profile/[0/XX]/settings.ini
     *
     * File Content: {[key][value]}
     *               {[key][value]}
     */
    final static class AudioProfileSettings {

        private static AudioProfileSettings sInstance = null;
        private static final String SYSTEM_DIR = "system";
        private static final String AUDIO_PROFILE_DIR = "audio_profile";
        private static final String FILE = "settings.ini";
        private AtomicFile mFile = null;
        private static final Map<String,String> DATAS = new Hashtable<String,String>();
        private static final String PROFILE_PREFIX_CONTENT = "{[";
        private static final String PROFILE_MIDDLE_CONTENT = "][";
        private static final String PROFILE_SUFFIX_CONTENT = "]}";
        private int mCurrentUser = UserHandle.USER_OWNER;

        private AudioProfileSettings(int userId){
            mCurrentUser = userId;
            File directory = Environment.getDataDirectory();
            File systemDir = new File(directory, SYSTEM_DIR);
            File syncDir = new File(systemDir,AUDIO_PROFILE_DIR);
            File realFile = new File(syncDir, String.valueOf(mCurrentUser));
            mFile = new AtomicFile(new File(realFile,FILE));
            DATAS.clear();
            if(isFileExisted()){
                InputStream is = null;
                try{
                    ...
                    while((content = reader.readLine()) != null){
                        Log.d(TAG,"AudioProfileSettings() content = " + content);
                        if(content == ""){
                            continue;
                        }
                        final int posOffset = 2;
                        int startPos = content.indexOf(PROFILE_PREFIX_CONTENT);
                        int middlePos = content.indexOf(PROFILE_MIDDLE_CONTENT);
                        int endPos = content.indexOf(PROFILE_SUFFIX_CONTENT);
                        if(startPos < 0
                          || middlePos <= startPos
                          || (endPos <= startPos
                          || endPos <= middlePos)){
                        ...
                        String key = content.substring(startPos+posOffset, middlePos);
                        String value = content.substring(middlePos+posOffset, endPos);
                        //Log.d(TAG,"AudioProfileSettings() key = " + key + ", value = " + value);
                        DATAS.put(key,value);
                    }
                } catch(IOException e){
                    e.printStackTrace();
                ...
                if(!systemDir.exists()){
                    boolean mkSystemDirSuccess = systemDir.mkdir();
                    Log.d(TAG,"AudioProfileSettings() mkSystemDirSuccess = " + mkSystemDirSuccess);
                }
                if(!syncDir.exists()){
                    boolean mkSyncDirSuccess = syncDir.mkdir();
                    Log.d(TAG,"AudioProfileSettings() mkSyncDirSuccess = " + mkSyncDirSuccess);
                }
                 if(!realFile.exists()){
                    boolean mkRealFileSuccess = realFile.mkdir();
                    Log.d(TAG,"AudioProfileSettings() mkRealFileSuccess = " + mkRealFileSuccess);
                }
                File file = new File(realFile,FILE);
                ...
                Log.d(TAG,"AudioProfileSettings() createFile = " + createFile);
                mFile = new AtomicFile(file);
            }
        }

        public static AudioProfileSettings getInstance(int userId){
            if(sInstance == null){
                sInstance = new AudioProfileSettings(userId);
            }
            if(sInstance.mCurrentUser != userId){
                sInstance = new AudioProfileSettings(userId);
            }
            return sInstance;
        }
        ...
        private boolean isFileExisted(){
            boolean fileExists = mFile.getBaseFile().exists();
            Log.d(TAG,"isFileExisted() fileExists = " + fileExists);
            return fileExists;
        }

        protected boolean put(final String key, final String value){
            if(key == null || value == null){
                Log.d(TAG,"put() key is null or value is null");
                return false;
            }
            ...
            String oldValue = DATAS.get(key);
            if(oldValue == value){
                return true;
            }
            DATAS.put(key,value);
            boolean writeSucced = true;
            FileOutputStream fos = null;
            try{
                fos = mFile.startWrite();
                PrintStream ps = new PrintStream(fos);
                for(Map.Entry<String,String> entry : DATAS.entrySet()) {
                    String content = PROFILE_PREFIX_CONTENT
                        + entry.getKey()
                        + PROFILE_MIDDLE_CONTENT
                        + entry.getValue()
                        + PROFILE_SUFFIX_CONTENT;
        ...
        protected boolean remove(final List<String> keys){
            if(keys == null){
                return false;
            }
            int size = keys.size();
            ...
            for(String key : keys){
                String value = DATAS.remove(key);
                //Log.d(TAG,"remove() key = " + key + " value = " + value);
            }
            boolean remove = true;
            FileOutputStream fos = null;
            try{
    ...
    private boolean needUpgradeProfileSettings(){
        int upgraded = Settings.System.getIntForUser(mContentResolver,
                                                     PROFILE_UPGRADED_KEY,
                                                     0,
                                                     mUserId);
        Log.d(TAG,"needUpgradeProfileSettings() upgraded = " + upgraded);
        if(upgraded == PROFILE_UPGRADED_VALUE){
            Log.d(TAG,"needUpgradeProfileSettings() already upgraded");
            return false;
        }
        return true;
    }
    /**
     * To upgrade the audio profile settings from com.android.providers.settings to settings.ini.
     * It's called while device boot completed and user switched.
     */
    private boolean upgradeProfileSettings(){
        Log.d(TAG,"upgradeProfileSettings() start");
        final List<String> profileKeys =
            new ArrayList<String>(AudioProfileManager.MAX_PROFILES_COUNT);
        // add the predefined profile keys for old implementation in SettingsProvider
        profileKeys.add(AudioProfileManager.getProfileKey(Scenario.GENERAL));
        profileKeys.add(AudioProfileManager.getProfileKey(Scenario.SILENT));
        profileKeys.add(AudioProfileManager.getProfileKey(Scenario.MEETING));
        profileKeys.add(AudioProfileManager.getProfileKey(Scenario.OUTDOOR));
        // add the custom profile keys for old implementation in SettingsProvider
        int maxCustomKeys = AudioProfileManager.MAX_PROFILES_COUNT
                - AudioProfileManager.PREDEFINED_PROFILES_COUNT;
        for(int serial = 0; serial <maxCustomKeys; serial++){
            String customKey = AudioProfileManager.getProfileKey(Scenario.CUSTOM)
                + "_" + String.valueOf(serial);
            profileKeys.add(customKey);
        }

        String activeProfile = Settings.System.getStringForUser(mContentResolver,
            AudioProfileManager.KEY_ACTIVE_PROFILE,mUserId);
        if(activeProfile != null && activeProfile != ""){
            mProfileSettings.put(AudioProfileManager.KEY_ACTIVE_PROFILE,activeProfile);
        }

        String lastActiveProfile = Settings.System.getStringForUser(mContentResolver,
            AudioProfileManager.LAST_ACTIVE_PROFILE,mUserId);
        if(lastActiveProfile != null && lastActiveProfile != ""){
            mProfileSettings.put(AudioProfileManager.LAST_ACTIVE_PROFILE,lastActiveProfile);
        }

        String lastActiveProfileDeleted = Settings.System.getStringForUser(mContentResolver,
            AudioProfileManager.LAST_ACTIVE_CUSTOM_DELETED,mUserId);
        if(lastActiveProfileDeleted != null && lastActiveProfileDeleted != ""){
            mProfileSettings.put(AudioProfileManager.LAST_ACTIVE_CUSTOM_DELETED,
                lastActiveProfileDeleted);
        }

        String bessuroundState = Settings.System.getStringForUser(mContentResolver,
            AudioProfileManager.KEY_BESSUROUND_STATE,mUserId);
        if(bessuroundState != null && bessuroundState != ""){
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_STATE,bessuroundState);
        }

        String bessuroundMode = Settings.System.getStringForUser(mContentResolver,
            AudioProfileManager.KEY_BESSUROUND_MODE,mUserId);
        if(bessuroundMode != null && bessuroundMode != ""){
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_MODE,bessuroundMode);
        }

        for(int i=0;i<AudioProfileManager.MAX_PROFILES_COUNT;i++){
            String profile = profileKeys.get(i);
            Log.d(TAG,"upgradeProfileSettings()-profile = " + profile);
            List<String> keys = getAllOldKeys(profile);
            for(String key : keys){
                if(key == null || key == ""){
                    continue;
                }
                String value = Settings.System.getStringForUser(mContentResolver,key,mUserId);
                if(value == null || value == ""){
                    continue;
                }
                Log.d(TAG,"upgradeProfileSettings()-key=" + key + " , value=" + value);
                mProfileSettings.put(key,value);
            }
        }
        Log.d(TAG,"upgradeProfileSettings() Settings.System"
                   + " set KEY_PROFILE_UPGRADED as PROFILE_UPGRADED");
        Settings.System.putIntForUser(mContentResolver,
                                      PROFILE_UPGRADED_KEY,
                                      PROFILE_UPGRADED_VALUE,
                                      mUserId);
        Log.d(TAG,"upgradeProfileSettings() end");
        return true;
    }

    /**
     * read all the keys via speicify profile that stored in the
     * com.android.providers.settings database.
     */
    private List<String> getAllOldKeys(String profile){
        List<String> keys = new ArrayList<String>();
        keys.add(AudioProfileManager.getKey(profile));
        keys.add(AudioProfileManager.getProfileNameKey(profile));
        keys.add(profile + AudioProfileManager.SUFFIX_NOTIFICATION_USE_RING);
        keys.add(profile + AudioProfileManager.SUFFIX_LOCK_SCRREN);
        keys.add(profile + AudioProfileManager.SUFFIX_HAPTICFEEDBACK);
        keys.add(profile + AudioProfileManager.SUFFIX_DTMFTONE);
        keys.add(profile + AudioProfileManager.SUFFIX_SOUNDEFFECT);
        keys.add(AudioProfileManager.getVibrationKey(profile));
        keys.add(AudioProfileManager.getStreamVolumeKey(profile,
                    AudioProfileManager.STREAM_ALARM));
        keys.add(AudioProfileManager.getStreamVolumeKey(profile,
                    AudioProfileManager.STREAM_NOTIFICATION));
        keys.add(AudioProfileManager.getStreamVolumeKey(profile,
                    AudioProfileManager.STREAM_RING));
        // add for ringtone of none SIM card
        keys.add(AudioProfileManager.getStreamUriKey(profile,
                AudioProfileManager.TYPE_RINGTONE));
        keys.add(AudioProfileManager.getDataKey(
                    AudioProfileManager.getStreamUriKey(profile,
                    AudioProfileManager.TYPE_RINGTONE)));
        // add for ringtone of SIM card [N]. Speicify for multi SIM and SIM inserted
        int simCount = mTeleManager.getSimCount();
        for(int simId=1; simId<=simCount; simId++){
            keys.add(AudioProfileManager.getStreamUriKey(profile,
                AudioProfileManager.TYPE_RINGTONE, simId));
            keys.add(AudioProfileManager.getDataKey(
                    AudioProfileManager.getStreamUriKey(profile,
    ...
    private final BroadcastReceiver mUserSwitchReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i(TAG, "mUserSwitchReceiver receive action " + action + ", userId = " + userId);
            if (Intent.ACTION_USER_SWITCHED.equals(action)) {
                if(mUserId != userId){
                    mUserId = userId;
                    mProfileSettings = null;
                    mProfileSettings = AudioProfileSettings.getInstance(mUserId);
                    if(needUpgradeProfileSettings()){
                        upgradeProfileSettings();
                    }
                    reInit();
                } else {
                    if(needUpgradeProfileSettings()){
