...
public class FloatingMonitorPointerEventListener
    ...
    private final GestureDetector.OnGestureListener mOnGestureListener =
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2,
            ...
            if (mMotionMode == MOTION_MODE_DRAG) {
                MultiWindowProxy.getInstance().moveFloatingWindow((int)distanceX,
                    (int)distanceY);
            } else if (mResizeDirect != RESIZE_NONE) {
                MultiWindowProxy.getInstance().resizeFloatingWindow(mResizeDirect,
    ...
    public FloatingMonitorPointerEventListener(
        ...
        mGestureDetector = new GestureDetector(mContext,
            mOnGestureListener);
        ///  M: [ALPS01684303]. Disable long press, or
        /// cannot move window when long press then control bar.
        ...
        mEnabledResizeDirections = new int[1];
        mEnabledResizeDirections[0] = 16;
    }
    ...
    private void computeCtrlBarRegion() {
        ...
        if (displayManager == null){
            Slog.v(TAG, "[BMW]DisplayManager Service not ready!");
            return ;
        }

        if (displayManager.getDisplay(Display.DEFAULT_DISPLAY) == null){
            Slog.v(TAG, "[BMW]No default display!");
            return ;
        }
    ...
    private boolean checkResizeDirectionEnabled(int currentResizeDirect){
        /// M: Add for Split Mode @{
        if (MultiWindowProxy.isSplitMode()) {
            if (currentResizeDirect == (RESIZE_LEFT | RESIZE_DOWN)
                    || currentResizeDirect == (RESIZE_LEFT | RESIZE_UP)
                    || currentResizeDirect == (RESIZE_RIGHT | RESIZE_DOWN)
                    || currentResizeDirect == (RESIZE_RIGHT | RESIZE_UP)) {
                    return false;
            }
        }
        /// @}
