...
public class MtkPhoneWindowManager extends PhoneWindowManager {
    ...
    public void layoutWindowLw(WindowState win, WindowState attached) {
        ...
        /// M. BMW. For multi-display
        if (!win.isFloatingWindow() || !mIsDefaultDisplay) {
            ...
            Slog.d(TAG, "[BMW] layoutWindowLw floating win = " + win);
            ...
            /// M. [ALPS02049788][ALPS02072703]. Modify pf just when imputmethod shown
            /// M. [ALPS02107524][ALPS02124829] All windows which in IME target's stack
            /// should move position with IME target window
            if ((getInputMethodWindowVisibleHeightLw() > 0
                        && mCurrentInputMethodTargetWindow == win)
                 || (mCurrentInputMethodTargetWindow != null
                        && mCurrentInputMethodTargetWindow != win
                        && mFocusedApp == win.getAppToken()
                        && mCurrentInputMethodTargetWindow.isFocusStack()
                        && mCurrentInputMethodTargetWindow.isVisibleLw()
                        && mCurrentInputMethodWindow != null
                        && win.getSurfaceLayer() >
                            mCurrentInputMethodWindow.getSurfaceLayer())) {
                if (mCurrentInputMethodTargetWindow == win) {
                    pf.bottom = Math.min(mContentBottom, pf.bottom);
                } else if (attached == null) {
                    // M. [ALPS02107524] Apply IME target window frame to focused window
                    Slog.v(TAG, "[BMW]Special case,set pf/sf/vf/cf to window target's frame");
                    pf.set(mCurrentInputMethodTargetWindow.getFrameLw());
                    df.set(pf);
                    of.set(pf);
                    cf.set(pf);
                    vf.set(pf);
            ...
            if (true) Slog.v(TAG, "[BMW] Compute float frame " + attrs.getTitle()
                    ...
                    + " dcf=" + dcf.toShortString() + " sf=" + sf.toShortString()
                    + " floatRegion=" + floatRegion.toShortString());

            win.computeFrameLw(pf, df, of, cf, vf, dcf, sf, null);
        }
    }
    ...
    public void applyPostLayoutPolicyLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        if (win != null && win.isFloatingWindow()) {
            Slog.d(TAG, "[BMW] Floating window skips applyPostLayoutPolicyLw ");
            return;
        }
    ...
    public int focusChangedLw(WindowState lastFocus, WindowState newFocus) {
        ...
        /// M: BMW. When the focus window is belong to the floating window, enable
