...
public final class PowerManagerService extends SystemService
    ...
    public void systemReady(IAppOpsService appOps) {
        synchronized (mLock) {
            ...
            String power_mode = SystemProperties.get("persist.sys.power.mode");
            if (power_mode.equals("0")) {
                powerHintInternal(PowerManager.POWER_HINT_PERFORMANCE_BOOST, 0);
            } else if (power_mode.equals("1")) {
                powerHintInternal(PowerManager.POWER_HINT_BALANCE, 0);
            } else if (power_mode.equals("2")) {
                powerHintInternal(PowerManager.POWER_HINT_POWER_SAVING, 0);
            } else {
                Slog.d(TAG, "power mode not exist");
            }
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY
            ...
            if (mWakefulness == WAKEFULNESS_AWAKE
                ...
                if(mScreenOffTimeoutSetting != 0) {   // 添加永不休眠
    ...
    private void powerHintInternal(int hintId, int data) {
        ...
        switch (hintId) {
            case PowerManager.POWER_HINT_POWER_SAVING:
                AalUtils.getInstance().setEnabled(mContext, true);
                AalUtils.getInstance().setAalMode(mContext, AalUtils.AAL_MODE_LOWPOWER);
                break;
            case PowerManager.POWER_HINT_PERFORMANCE_BOOST:
                AalUtils.getInstance().setEnabled(mContext, true);
                AalUtils.getInstance().setAalMode(mContext, AalUtils.AAL_MODE_PERFORMANCE);
                break;
            case PowerManager.POWER_HINT_BALANCE:
                AalUtils.getInstance().setEnabled(mContext, true);
                AalUtils.getInstance().setAalMode(mContext, AalUtils.AAL_MODE_BALANCE);
                break;
            default:
                break;
        }
