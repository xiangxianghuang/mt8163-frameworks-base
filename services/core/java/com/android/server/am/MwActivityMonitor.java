/*
 * Copyright Statement: This software/firmware and related documentation
 * ("MediaTek Software") are protected under relevant copyright laws. The information
 * contained herein is confidential and proprietary to MediaTek Inc. and/or its licensors.
 * Without the prior written permission of MediaTek inc. and/or its licensors, any
 * reproduction, modification, use or disclosure of MediaTek Software, and information
 * contained herein, in whole or in part, shall be strictly prohibited. MediaTek Inc. (C)
 * 2014. All rights reserved. BY OPENING THIS FILE, RECEIVER HEREBY UNEQUIVOCALLY
 * ACKNOWLEDGES AND AGREES THAT THE SOFTWARE/FIRMWARE AND ITS DOCUMENTATIONS
 * ("MEDIATEK SOFTWARE") RECEIVED FROM MEDIATEK AND/OR ITS REPRESENTATIVES ARE PROVIDED TO
 * RECEIVER ON AN "AS-IS" BASIS ONLY. MEDIATEK EXPRESSLY DISCLAIMS ANY AND ALL WARRANTIES,
 * EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTIES OF
 * MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE OR NONINFRINGEMENT. NEITHER DOES
 * MEDIATEK PROVIDE ANY WARRANTY WHATSOEVER WITH RESPECT TO THE SOFTWARE OF ANY THIRD
 * PARTY WHICH MAY BE USED BY, INCORPORATED IN, OR SUPPLIED WITH THE MEDIATEK SOFTWARE,
 * AND RECEIVER AGREES TO LOOK ONLY TO SUCH THIRD PARTY FOR ANY WARRANTY CLAIM RELATING
 * THERETO. RECEIVER EXPRESSLY ACKNOWLEDGES THAT IT IS RECEIVER'S SOLE RESPONSIBILITY TO
 * OBTAIN FROM ANY THIRD PARTY ALL PROPER LICENSES CONTAINED IN MEDIATEK SOFTWARE.
 * MEDIATEK SHALL ALSO NOT BE RESPONSIBLE FOR ANY MEDIATEK SOFTWARE RELEASES MADE TO
 * RECEIVER'S SPECIFICATION OR TO CONFORM TO A PARTICULAR STANDARD OR OPEN FORUM.
 * RECEIVER'S SOLE AND EXCLUSIVE REMEDY AND MEDIATEK'S ENTIRE AND CUMULATIVE LIABILITY
 * WITH RESPECT TO THE MEDIATEK SOFTWARE RELEASED HEREUNDER WILL BE, AT MEDIATEK'S OPTION,
 * TO REVISE OR REPLACE THE MEDIATEK SOFTWARE AT ISSUE, OR REFUND ANY SOFTWARE LICENSE
 * FEES OR SERVICE CHARGE PAID BY RECEIVER TO MEDIATEK FOR SUCH MEDIATEK SOFTWARE AT
 * ISSUE. The following software/firmware and/or related documentation ("MediaTek
 * Software") have been modified by MediaTek Inc. All revisions are subject to any
...
public class MwActivityMonitor {
    ...
    private int mMaximumFrontFloatingSize = MultiWindowProxy.getMaxFrontFloatingSize();
            ...
            switch (msg.what) {
                case SHOW_ONE_ACTIVITY_AT_GALLERY3D_MSG: {
                    Toast toast = Toast.makeText(
                            mContext,
                            "Cannot use Gallery,Camera or Video Player at the same time",
                ...
                case SHOW_MAXIMUM_FRONT_FLOATING_MSG: {
                    Toast toast = Toast.makeText(
                            mContext,
                            "Maximum " + mMaximumFrontFloatingSize
                            + " floating windows!",
                            /*mContext.getResources().getString(
                ...
                case MSG_MW_MAX_RESTORE: {
                    ...
                    synchronized (mService) {
                        if (mService.mLruProcesses.contains(obj.mAr.app)
                                    && obj.mAr.app == obj.mPr) {
                            Log.v(TAG, "ACT-reset the process max/restore status for app : "
                                    + obj.mAr.app);
                            obj.mAr.app.inMaxOrRestore = false;
                        } else {
                            Log.d(TAG, "obj.mAr.app != obj.mPr, obj.mAr.app:" + obj.mAr.app
                                    + ", obj.mPr:" + obj.mPr);
                        }
                    }
                    break;
     ...
     * @return
     */
    public int blockByRunningActivity(String processName, ProcessRecord callerApp,
            Intent nextIntent, ActivityRecord[] outRunningAc, ActivityInfo aInfo) {
        if (DEBUG) {
            Log.d(TAG, "blockByRunningActivity processName = " + processName + ", callerApp = "
                    + callerApp + ", nextIntent = " + nextIntent);
        }

        // [ALPS02039971], For app activity start by same process , do not block it.
        mCallerApp = callerApp;

        String nextComponentName = nextIntent.getComponent().flattenToShortString();
        ...
        for (int i = N - 1; i >= 0; i--) {
            ...
            if (callerApp != null && runningAc.processName.equals(callerApp.processName)) {
                 ...
                 */
                Log.v(TAG,
                        "blockByRunningActivity : " + " caller apps are same "
                                + Debug.getCallers(4));
            ...
            if (nextComponentName.contains(runningAc.shortComponentName)) {
                Log.v(TAG,
                        "blockByRunningActivity : " + " 2 shortComponentNames are same "
                                + Debug.getCallers(4));
                return NG_SAME_COMPONENT_NAME;
            }

            if (runningAc.processName != null && runningAc.processName.equals(processName)
                ...
                if (runningAc.task.stack.findActivityLocked(nextIntent, aInfo) != null) {
                    Log.v(TAG, "blockByRunningActivity : "
                            + " 3 exist same activity in resumed stack " + Debug.getCallers(4));
                    return NG_SAME_TASK;
                    ...
                    Log.v(TAG,
                            "blockByRunningActivity : " + processName
                                    + " has activity for resumed : " + runningAc
                                    + ". Therefore, don't be launched " + ", callers = "
                                    + Debug.getCallers(4));
                    mMwHandler.sendEmptyMessage(SHOW_ONE_ACTIVITY_AT_GALLERY3D_MSG);
    ...
    boolean isBlockedByFloatingStackSizeWhenRestore() {
        final ArrayList<ActivityStack> frontFloatStacks = mService.mStackSupervisor
                .getFrontFloatStacks(Display.DEFAULT_DISPLAY);
        mMaximumFrontFloatingSize = MultiWindowProxy.getMaxFrontFloatingSize();

        int frontStackSize = 0;
        for (int n = 0; n <= frontFloatStacks.size() - 1; n++) {
            ActivityStack tmpStack = frontFloatStacks.get(n);
            ActivityRecord top = tmpStack.topRunningActivityLocked(null);
            if (top == null) {
                MultiWindowProxy.getInstance().resetStackPosition(tmpStack.mStackId);
            }
            if (top !=  null) {
                frontStackSize += 1;
            }
    ...
    /**
     * After finding the target stack, the function should be called to check the floating
     * stack should be blocked or not.
     *
     * @param stack :
     * @return true : Block the stack. false: Don't block the stack.
     */
    boolean isBlockedByFloatingStackSize(ActivityStack stack) {
        if (stack == null) {
            return false;
        }
        ...
        for (int n = 0; n <= frontFloatStacks.size() - 1; n++) {
            ...
            if (top !=  null) {
                frontStackSize += 1;
                if (stack == tmpStack) {
                    MultiWindowProxy.getInstance().computeStackPosition(stack.mStackId);
                    return false;
                }
            }
        }
    ...
    public int blockByFloatingStackSize(ActivityRecord r) {
        ...
        frontFloatingStacks = mService.mStackSupervisor
                .getFrontFloatStacks(Display.DEFAULT_DISPLAY);
        ...
        Log.d(TAG, "blockByFloatingStackSize floating stack size = " + frontStackSize);
            ...
            if (r != null) {
                if ((r.intent.getFlags() & Intent.FLAG_ACTIVITY_FLOATING) != 0) {
                    ...
                    for (int i = frontStackSize - 1; i >= 0; i--) {
                        ActivityRecord ar = frontFloatingStacks.get(i).findTaskLocked(r);
                        if (ar != null) {
                            blocked = false;
                            break;
                        }
                    ...
                    // [ALPS02039971] ,If APP using to activity to start it main UI,we
                    // can not block it.
                    ...
                    for (int n = frontStackSize - 1; n >= 0; n--) {
                        ...
                        if (ar != null) {
                            ...
                            // for specia apk, activity in different process. eg.
                            // Downloads apk.
                    ...
                    // [ALPS02063467], If topActivity is null, activity is in pending
                    // finish list. So its stack is not in front later.
                    if (frontStackSize - noTopActvityInFrontStackSize < mMaximumFrontFloatingSize) {
                        blocked = false;
                    }

                    // [ALPS02039971], If Caller APP and target APP in same process,do
                    // not blocking it.
