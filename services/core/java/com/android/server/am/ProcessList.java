...
final class ProcessList {
        ...
        if (SystemProperties.get("ro.mtk_gmo_ram_optimize").equals("1")) {
            ...
            if (mTotalMemMb <= LOW_LEVEL_SIZE) {
                ...
                mOomMinFree = new int[] {
                    ...
                    81920, // 80 * 1024 (ADJ 9 -> 80MB) (based on performance test)
                    102400  // 100 * 1024 (ADJ 15 -> 100MB) (based on performance test)
                };
            } else {
                mOomAdj = new int[] {
                    FOREGROUND_APP_ADJ,
    ...
    public static long computeNextPssTime(int procState, boolean first, boolean test,
        ...
        if (procState == -1) {
            ...
            Slog.w(TAG, "[computeNextPssTime] Unintialized procState = " + procState);
            Thread.dumpStack();
        }
        /// @}
