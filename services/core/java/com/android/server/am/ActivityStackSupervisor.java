...
public final class ActivityStackSupervisor implements DisplayListener {
    ...
    boolean isFrontStack(ActivityStack stack) {
        ...
        if (MultiWindowProxy.isSupported() && isFloatingStack(stack.mStackId)) {
            if (getFrontFloatStacks(stack.mDisplayId) != null
                    && getFrontFloatStacks(stack.mDisplayId).contains(stack)) {
                return true;
            }
            return false;
        }
    ...
    void moveHomeStack(boolean toFront, String reason, ActivityStack lastFocusedStack) {
        ...
        /// M: BMW @{
        if (MultiWindowProxy.isSupported()) {
            boolean homeInFront = isFrontStack(mHomeStack);
            if (toFront != homeInFront) {
                if (DEBUG_STACK) Slog.d(TAG_STACK, "[BMW]moveHomeTask: toFront ="
                        + toFront + ", topStack old="
                        + ((lastFocusedStack != null) ? lastFocusedStack : stacks.get(topNdx)));
                stacks.remove(mHomeStack);
                stacks.add(toFront ? topNdx : 0, mHomeStack);
                if (DEBUG_STACK) Slog.d(TAG_STACK, "[BMW]moveHomeTask: topStack new="
                        + stacks.get(topNdx));
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (MultiWindowProxy.isSupported()) {
            // M. [ALPS02473353] add float flag If launched from floating stack @{
            ArrayList<ActivityStack> stackList = getStacks();
            for (int i = 0; i < stackList.size(); i++) {
                ActivityStack stack = stackList.get(i);
                ActivityRecord activity = stack.topRunningActivityLocked(null);
                if (activity != null) {
                    ProcessRecord app = activity.app;
                    if (app != null && app == callerApp && callingPackage != null
                            && callingPackage.equals(activity.packageName)) {
                        if (isFloatingStack(stack.mStackId)
                                && MultiWindowProxy.getInstance().inWhiteList(r.packageName)) {
                            addFlag(r, Intent.FLAG_ACTIVITY_FLOATING);
                        }
                    }
                }
            }
    ...
    ActivityStack computeStackFocus(ActivityRecord r, boolean newTask) {
        ...
        if (!mLeanbackOnlyDevice &&
            ...
            /// M: BMW. Create a new floating stack and make sure normal
            /// activity won't start at floating stack @{
            /// M: [ALPS02071575] Check packagename if disbled for floating
            if (MultiWindowProxy.isSupported()) {
                if ((r.intent.getFlags() & Intent.FLAG_ACTIVITY_FLOATING) != 0
                        && MultiWindowProxy.getInstance().inWhiteList(r.packageName)) {
                    // Time to create a floating app stack for this user.
                    int stackId = createFloatingStackOnDisplay(getNextStackId(),
                                    Display.DEFAULT_DISPLAY);
                    Slog.d(TAG, "[BMW]computeStackFocus: New floating stack r="
                                    + r + " stackId=" + stackId);
                    ...
                    for (int stackNdx = homeDisplayStacks.size() - 1; stackNdx >= 0; --stackNdx) {
                        ActivityStack stackTmp = homeDisplayStacks.get(stackNdx);
                        if (!stackTmp.isHomeStack() && !isFloatingStack(stackTmp.mStackId)) {
                            if (DEBUG_FOCUS || DEBUG_STACK) Slog.d(TAG,
                                "[BMW]computeStackFocus: Setting focused stack=" + stackTmp);
                            return stackTmp;
                        }
                    }
                    ActivityStack newStack = createStackOnDisplay(getNextStackId(),
                            Display.DEFAULT_DISPLAY);
                    Slog.d(TAG, "[BMW]computeStackFocus: New stack (from a floating stack) r="
                                + r + " stackId=" + newStack.mStackId);
                    return newStack;
                }
            }
            /// @}
    ...
    final int startActivityUncheckedLocked(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        if (((launchFlags & Intent.FLAG_ACTIVITY_NEW_TASK) != 0 &&
            ...
            if (inTask == null && r.resultTo == null) {
                ...
                if (intentActivity != null) {
                    ...
                    /// M: BMW. [ALPS02056018]. If find activity in floating stack,
                    /// add its task to mRecentTasks again.  Because task may be removed from
                    /// mRecentTasks by starting other task that has same affinity. @{
                    if (MultiWindowProxy.isSupported()) {
                        if (targetStack.isFloatingStack()) {
                            if (DEBUG_TASKS) Slog.i(TAG, "[BMW] find task in floating ,"
                                    + " add to Recent again.");
                            mRecentTasks.addLocked(intentActivity.task);
                        }
                        /// M: BMW. Adjust intent and stack
                        adjustIntentAndStack(targetStack, r);
                        /// [ALPS02521887] Check if the stack should be blocked. @{
                        if (mService.mMwActivityMonitor.isBlockedByFloatingStackSize(targetStack)) {
                            Slog.i(TAG, "[BMW] startActivityUnchecked: "
                                    + "blocked by floating stack size");
                            return ActivityManager.START_SUCCESS;
                        }
                        /// @}
                    }
        ...
        if (r.resultTo == null && inTask == null && !addingToTask
            ...
            if (MultiWindowProxy.isSupported()) {
                ...
                /// [ALPS02521887] Check if the stack should be blocked.@{
                if (mService.mMwActivityMonitor.isBlockedByFloatingStackSize(targetStack)) {
                    Slog.i(TAG, "startActivityUnchecked: blocked by floating stack size");
                    return ActivityManager.START_SUCCESS;
                }
                /// @}
            }
            ...
            if (isLockTaskModeViolation(r.task)) {
                /// M: Show lock task toast when violation
                showLockTaskToast();
            ...
            /// M: BMW. [ALPS02521887] Check if the stack should be blocked. @{
            if (MultiWindowProxy.isSupported()) {
                if (mService.mMwActivityMonitor.isBlockedByFloatingStackSize(inTask.stack)) {
                    Slog.i(TAG, "startActivityUncheced: blocked by floating stack size");
                    return ActivityManager.START_SUCCESS;
                }
            }
            /// @}
    ...
    boolean resumeTopActivitiesLocked(ActivityStack targetStack, ActivityRecord target,
        ...
        /// M: BMW. [ALPS01740467], update sticky stack status. @{
        if (MultiWindowProxy.isSupported()) {
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ArrayList<ActivityStack> stacks = mActivityDisplays
    ...
    private boolean restoreRecentTaskLocked(TaskRecord task) {
        ...
        if (mLeanbackOnlyDevice) {
            ...
            for (int stackNdx = homeDisplayStacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                /// M: BMW. [ALPS02039490]. Only find App stack,
                /// and we will configure floating stack for floating task later. @{
                if (MultiWindowProxy.isSupported() && tmpStack.isFloatingStack()) {
                    continue;
                }
                /// @}
    ...
    boolean switchUserLocked(int userId, UserState uss) {
        /// M: BMW. [ALPS02510139]. Move all floating stacks to back
        /// when switch to normal user @{
        if (MultiWindowProxy.isSupported() && userId != UserHandle.USER_OWNER) {
            moveAllFloatingStacksToBackLocked(Display.DEFAULT_DISPLAY);
        }
        /// @}
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                /// M: BMW. [ALPS02605540]. Not change task order for backFloating stack @{
                if (MultiWindowProxy.isSupported()
                        && isFloatingStack(stack.mStackId)
                        && !isFrontStack(stack)) {
                    continue;
        ...
        if (stack.isOnHomeDisplay()) {
            ...
            /// M: BMW. [ALPS02605429]. Set focused activity when switch user @{
            if (MultiWindowProxy.isSupported()) {
                final ActivityRecord top = topRunningActivityLocked();
                if (top != null) {
                    mService.setFocusedActivityLocked(top, "switchUserOnHomeDisplay");
                }
            }
            /// @}
        } else {
        ...
        if (MultiWindowProxy.isSupported()) {
            ...
            for (int displayNdx = 0; displayNdx < numDisplays; ++displayNdx) {
                ...
                for (int stackNdx = frontFloatstacks.size() - 1; stackNdx >= 0; --stackNdx) {
                    ...
                    pw.println(frontFloatstacks.get(stackNdx).toString());
                }
                ...
                for (int stackNdx = backFloatstacks.size() - 1; stackNdx >= 0; --stackNdx) {
                    pw.print(prefix); pw.print(prefix);
                    pw.println(backFloatstacks.get(stackNdx).toString());
                }
            }
            /// M: BMW @}
        }
    ...
    class ActivityDisplay {
        ...
        private final ArrayList<ActivityStack> mFrontFloatingStacks =
                new ArrayList<ActivityStack>();
        /// M: BMW. Back floating stacks which means the floating stack after "moveTaskToBack"
        private final ArrayList<ActivityStack> mBackFloatingStacks =
                new ArrayList<ActivityStack>();
        ...
        void detachActivitiesLocked(ActivityStack stack) {
            ...
            /// M: BMW. @{
            if (MultiWindowProxy.isSupported() && isFloatingStack(stack.mStackId)) {
                MultiWindowProxy.getInstance().removeFloatingStack(stack.mStackId);
    ...
    */
    ...
    private int createFloatingStackOnDisplay(int stackId, int displayId) {
        ...
        MultiWindowProxy.getInstance().createFloatingStack(stackId);
        activityContainer.attachToDisplayLocked(activityDisplay);
    ...
    void moveFloatingStackToFrontLocked(ActivityStack stack) {
        ...
        frontFloatStacks.add(stack);
        stack.mStacks.remove(stack);
        stack.mStacks.add(stack);
        if (stack.mResumedActivity != null) {
            mService.setFocusedActivityLocked(stack.mResumedActivity,
                    "moveFloatingStackToFrontLocked");
        }
    }

    /// M: BMW.
    ActivityStack getNextFrontFloatingStackLocked(ActivityStack prev) {
        ArrayList<ActivityStack> stacks = getFrontFloatStacks(prev.mDisplayId);
        if (stacks != null) {
    ...
    /// M: BMW.
    boolean adjustFocusToNextFrontFloatingStackLocked(ActivityStack inStack, String reason) {
        final ActivityStack stack = inStack;
        final String myReason = reason + " adjustFocusToNextFrontFloatingStackLocked";
        if (stack == null) {
            return false;
        }
    ...
    /// At floating case, we should check if there is floating stack
    /// need to be shown before moving home to top
    /// 1) find the next front floating stack except prev stack
    /// 2) if prev is floating stack, only adjust focus to next front floating stack.
    /// 3) if prev is app stack, adjust stack order and focus.
    boolean moveNextFloatingStackToTopLocked(int returnTaskType,
            ActivityStack prev) {
        // If prev is null, just return false for resuming home.
        if (prev == null) {
            Slog.e(TAG, "[BMW] moveNextFloatingStackToTopLocked prev = null");
            return false;
        }
        // Find next front floating stack. If there is no floating stack,
        // just return false for resuming home.
        ActivityStack next = getNextFrontFloatingStackLocked(prev);
        if (next ==  null) {
            Slog.e(TAG, "[BMW] moveNextFloatingStackToTopLocked next = null");
            return false;
        }
        // [ALPS02510139]. Should't move floating stack belong to other user
        // TODO, multitask in floating stack?
        if (next.topTask() != null && next.topTask().userId != mCurrentUser) {
            Slog.e(TAG, "[BMW] moveNextFloatingStackToTopLocked not current user");
            return false;
        }

        if (DEBUG_FOCUS || DEBUG_STACK) {
            Slog.d(TAG, "[BMW] moveNextFloatingStackToTopLocked "
                    + "next = " + next + " prev = " + prev);
        }

        // For app stack , adjust stack order and focus
        // TODO: For app stack , adjust stack order and focus
        // Should move home to front first.
        moveHomeStack(true, "moveNextFloatingStackToTopLocked");
        mHomeStack.moveHomeStackTaskToTop(returnTaskType);

        ArrayList<ActivityStack> frontFloatStacks =
               new ArrayList<ActivityStack>(getFrontFloatStacks(next.mDisplayId));
        for (int i = 0; i < frontFloatStacks.size(); i++) {
            ActivityStack stack = frontFloatStacks.get(i);
            if (stack != prev) {
                TaskRecord task = stack.topTask();
                moveFloatingStackToFrontLocked(stack);
                if (task != null) {
                    mWindowManager.moveTaskToTop(task.taskId);
                }
            }
        }
        // [ALPS02444882] Pause back and resume top.
        resumeTopActivitiesLocked();
        return true;
    }
    ...
    void moveFloatingStackToBackLocked(ActivityStack stack) {
        ...
        stacks.add(0, stack);
        // Add for Split Mode
        MultiWindowProxy.getInstance().resetStackPosition(stack.mStackId);

        /// [ALPS02523277] [ALPS02541036] Should update focused stack and activity
        /// when moving floating stack to back. @{
        mLastFocusedStack = mFocusedStack;
        mFocusedStack = stacks.get(stacks.size() - 1);
        final ActivityRecord top = topRunningActivityLocked();
        if (top != null) {
            mService.setFocusedActivityLocked(top, "moveFloatingStackToBack");
        }
        /// @}
    }

    /// M: BMW. Find an app stack (none-floating) from top to buttom.
    ActivityStack getAppStackLocked(int displayId) {
    ...
    /// M: BMW. Move all tasks of the floating stack to app stack.
    /// If there is no app stack, create one.
    ActivityStack moveFloatingStackToAppStackLocked(
            ActivityStack floatingStack, boolean doResumeTop) {
        ...
        ActivityStack appStack = getAppStackLocked(displayId);

        boolean taskReturnToHome = true;
        // If appStack is in front, should return to app.
        if (appStack != null && isFrontStack(appStack)) {
            taskReturnToHome = false;
        }

        // If there is no app stack, create one.
        if (appStack == null) {
            // Time to create an app stack for floating stack to move
            appStack = createStackOnDisplay(getNextStackId(), displayId);
            if (DEBUG_FOCUS || DEBUG_STACK) {
                Slog.d(TAG, "[BMW] moveFloatingStackToAppStackLocked: "
                        + "Create new app stack stackId=" + appStack.mStackId);
            }
        }

        if (appStack != null) {
            // Move app stack to front first.
            appStack.moveToFront("moveFloatingStackToAppStackLocked");
            ...
            for (int i = 0; i < tasks.size(); i++) {
                ...
                task.stack.removeTask(task, "moveFloatingStackToAppStackLocked");
                appStack.addTask(task, true, true);
                mWindowManager.addTask(task.taskId, appStack.mStackId, true);
            }

            // Change taskToReturnTo for appStack's Top task
            if (appStack.topTask() != null) {
                if (taskReturnToHome) {
                    appStack.topTask().setTaskToReturnTo(HOME_ACTIVITY_TYPE);
                } else {
                    appStack.topTask().setTaskToReturnTo(APPLICATION_ACTIVITY_TYPE);
                }
            }

            mService.setFocusedActivityLocked(targetActivity,
                    "moveFloatingStackToAppStackLocked");
            updateProcessStatus(targetActivity);
    ...
    void moveActivityTaskToFloatingStackLocked(ActivityRecord targetActivity) {
        ...
        int stackId = createFloatingStackOnDisplay(getNextStackId(),
                Display.DEFAULT_DISPLAY);
        if (DEBUG_FOCUS || DEBUG_STACK) {
            Slog.d(TAG, "[BMW] moveActivityTaskToFloatingStackLocked: "
                    + "Create new floating stack:stackId=" + stackId);
        }
        ActivityStack floatingStack = getStack(stackId);
        if (floatingStack == null)
            return;

        TaskRecord targetTask = targetActivity.task;
        ActivityStack oldStack = targetActivity.task.stack;
        boolean needResumeHome = targetTask.isOverHomeStack()
                || (oldStack.numTasks() <= 1 && oldStack.isOnHomeDisplay());

        if (floatingStack != null) {
            // Move the floating stack to front first.
            floatingStack.moveToFront("moveActivityTaskToFloatingStackLocked");
            // Move all tasks to floating stack.
            for (int i = 0; i < targetTask.mActivities.size(); i++) {
                ActivityRecord act = targetTask.mActivities.get(i);
                // Update activity's status (ex: intent flag, fullscreen)
            ...
            targetActivity.task.stack.removeTask(targetActivity.task,
                    "moveActivityTaskToFloatingStackLocked");
            floatingStack.addTask(targetActivity.task, true, true);
            mWindowManager.addTask(targetActivity.task.taskId,
                    floatingStack.mStackId, true);
        }

        //Change taskReturnTo for floatingStack's Top task
        if (floatingStack.topTask() != null) {
            floatingStack.topTask().setTaskToReturnTo(ActivityRecord.NULL_ACTIVITY_TYPE);
        }

        // If taskReturnTo is Home, then need resume home after moving stack.
        if (needResumeHome) {
            moveHomeAndFloatStackTaskToTopLocked(HOME_ACTIVITY_TYPE);
        }
        // Update focus activity
        mService.setFocusedActivityLocked(targetActivity,
                "moveActivityTaskToFloatingStackLocked");
        updateProcessStatus(targetActivity);
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int i = frontFloatStacks.size() - 1; i >= 0; i--) {
                ...
                if (isStickStack(floatingStack.mStackId)) {
                    if (DEBUG_FOCUS || DEBUG_STACK) {
                        Slog.v(TAG, "[BMW] keepStickyStackLocked: stack = "
                                + floatingStack + " topTask = " + floatingStack.topTask());
                    }
            ...
            for (int i = 0; i < stickyStacks.size(); i++) {
                ...
                TaskRecord task = stickyStack.topTask();
                if (task != null) {
                    mWindowManager.moveTaskToTop(task.taskId);
                }
            }
    ...
    /// M: BMW.
    void moveHomeAndFloatStackTaskToTopLocked(int returnTaskType) {
        if (DEBUG_TASKS || DEBUG_STACK) Slog.v(TAG, "moveHomeAndFloatStackToTopLocked");
        // Should move home to front first & adjust focus
        moveHomeStack(true, "[BMW] moveHomeAndFloatStackToTopLocked");
        mHomeStack.moveHomeStackTaskToTop(returnTaskType);
        // TODO: Should move home task (WindowManager).?

        ArrayList<ActivityStack> frontFloatStacks =
               new ArrayList<ActivityStack>(getFrontFloatStacks(Display.DEFAULT_DISPLAY));

        for (int i = 0; i < frontFloatStacks.size(); i++) {
            ActivityStack stack = frontFloatStacks.get(i);
            // M: [ALPS02464467]. Move floating stack to front and update focus stack
            stack.moveToFront("moveHomeAndFloatStackToTopLocked");
        }
    }

    /// M: BMW. Resume target stack top activity when move from
    /// floating to normal stack or move from normal app to floating stack.
    boolean resumeTargetTopActivitiesLocked(ActivityStack targetStack,
        ...
        /// The resume sequence must be from bottom to top.
        for (int stackNdx = 0; stackNdx < stackSize; ++stackNdx) {
            final ActivityStack stack = stacks.get(stackNdx);
            if (isFrontStack(stack)) {
                if (DEBUG_FOCUS || DEBUG_STACK) {
                    Slog.v(TAG, "[BMW] resumeTargetTopActivitiesLocked resume:" + stack);
                }
                if (stack == targetStack) {
                    result = targetStack.resumeTopActivityLocked(target, targetOptions);
                } else {
    ...
    void switchMultiWindowMode() {
        ArrayList<ActivityStack> frontFloatStacks = new ArrayList<ActivityStack>(
                getFrontFloatStacks(Display.DEFAULT_DISPLAY));
        int maxSize = MultiWindowProxy.getMaxFrontFloatingSize();
        if (frontFloatStacks.size() > maxSize) {
            for (int i = 0; i < frontFloatStacks.size() - maxSize; i ++) {
                moveFloatingStackToBackLocked(frontFloatStacks.get(i));
            }
            resumeTopActivitiesLocked(null, null, null);
        }
        ArrayList<ActivityStack> Stacks = getFrontFloatStacks(Display.DEFAULT_DISPLAY);
        for (int i = 0; i < Stacks.size(); i ++) {
            MultiWindowProxy.getInstance().resizeAndMoveStack(Stacks.get(i).mStackId);
        }
    }

    /// M: BMW.
    ArrayList<ActivityStack> getFrontFloatStacks(int displayId) {
        return mActivityDisplays.get(displayId).mFrontFloatingStacks;
    }

    /// M: BMW.
    ArrayList<ActivityStack> getBackFloatStacks(int displayId) {
        return mActivityDisplays.get(displayId).mBackFloatingStacks;
    }

    /// M: BMW. Check if the stack is floating.
    ...
    /// M: BMW. Check if the stack is sticky.
    ...
    /// M: BMW.
    private void clearFlag(ActivityRecord r, int flag) {
        r.intent.setFlags(r.intent.getFlags() & ~flag);
        if (r.task != null) {
            r.task.intent.setFlags(r.intent.getFlags() & ~flag);
        }
    }

    /// M: BMW.
    private void addFlag(ActivityRecord r, int flag) {
        r.intent.addFlags(flag);
        if (r.task != null) {
            r.task.intent.addFlags(flag);
        }
    }

    /// M: BMW. Adjust intent and target stack.
    private void adjustIntentAndStack(ActivityStack targetStack, ActivityRecord r) {
        boolean targetIsFloating = isFloatingStack(targetStack.mStackId);
        boolean isInWhiteList = MultiWindowProxy.getInstance()
                .inWhiteList(r.packageName);
        boolean needAddFlag = targetIsFloating && isInWhiteList;
        if (needAddFlag) {
            addFlag(r, Intent.FLAG_ACTIVITY_FLOATING);
        } else {
            clearFlag(r, Intent.FLAG_ACTIVITY_FLOATING);
            if (targetIsFloating && !isInWhiteList) {
                targetStack = moveFloatingStackToAppStackLocked(targetStack, false);
            }
        }
    }

    /// M: BMW. Dump all stack info for debug.
    void dumpStackInfo(int displayId, String msg) {
        if (MultiWindowProxy.isSupported()) {
            if (DEBUG_TASKS || DEBUG_STACK) {
                ArrayList<ActivityStack> stacks = mActivityDisplays.get(displayId).mStacks;
                ArrayList<ActivityStack> frontFloatStacks = getFrontFloatStacks(displayId);
                ArrayList<ActivityStack> backFloatStacks = getBackFloatStacks(displayId);
                Slog.v(TAG, "[BMW] DumpStackInfo:displayId = " + displayId + " msg:" + msg);
                Slog.v(TAG, "[BMW]              :stacks = " + stacks);
                Slog.v(TAG, "[BMW]              :front  = " + frontFloatStacks);
                Slog.v(TAG, "[BMW]              :back   = " + backFloatStacks);
                Slog.v(TAG, "[BMW]              :focus  = " + mFocusedStack);
                Slog.v(TAG, "[BMW]              :lastFo = " + mLastFocusedStack);
            }
        }
    }

    /// M: BMW
    void updateProcessStatus(ActivityRecord targetActivity) {
        if (targetActivity != null && targetActivity.app != null) {
            targetActivity.app.inMaxOrRestore = true;
            // Use a Handler to reset the process staus.
            mService.mMwActivityMonitor.resetProcessMiniMaxStatus(targetActivity,
                    targetActivity.app);
        } else {
            // This should not occur
            Slog.e(TAG, "[BMW] updateProcessStatus: This should not occur! "
                    + "targetActivity = " + targetActivity);
        }
    }

    /// M: BMW
    private void moveAllFloatingStacksToBackLocked(int displayId) {
        ArrayList<ActivityStack> frontFloatStacks = getFrontFloatStacks(displayId);
        ArrayList<ActivityStack> backFloatStacks = getBackFloatStacks(displayId);
        ArrayList<ActivityStack> stacks = mHomeStack.mStacks;
        if (frontFloatStacks.size() == 0) {
            return;
        }
        for (int i = 0; i < frontFloatStacks.size(); i++) {
            ActivityStack stack = frontFloatStacks.get(i);
            if (isStickStack(stack.mStackId)) {
                MultiWindowProxy.getInstance().
                    stickWindow(stack.topActivity().appToken, false);
            }
            backFloatStacks.add(stack);
            stacks.remove(stack);
            stacks.add(0, stack);
