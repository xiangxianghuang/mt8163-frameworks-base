...
final class ActivityStack {
        ...
        if (isAttached()) {
            ...
            if (!homeStack) {
                ...
                if (MultiWindowProxy.isSupported()) {
                    final boolean floatingStack = isFloatingStack();

                    if (floatingStack) {
                        // FloatingStack: move to front stack
                        // (TODO consider sticky stack?)
                        backFloatStacks.remove(this);
                        frontFloatStacks.remove(this);
                        frontFloatStacks.add(this);
                    }
                }
                /// @}
            }
            ...
            if (MultiWindowProxy.isSupported()) {
                if (homeStack) {
                    /// Move all float stack to back when home stack to front
                    if (DEBUG_TASKS || DEBUG_STACK) {
                        Slog.v(TAG, "[BMW] moveToFront(homeStack), move all float stack to back",
            ...
            if (isOnHomeDisplay()) {
                ...
                if (MultiWindowProxy.isSupported() && isFloatingStack()) {
                    ActivityStack home = mStackSupervisor.getStack(HOME_STACK_ID);
                    boolean homeInFront = mStackSupervisor.isFrontStack(home);
                    mStackSupervisor.moveHomeStack(homeInFront, reason, lastFocusStack);
                } else {
            ...
            /// M: BMW. [ALPS02071228],update sticky stack status.@{
            if (MultiWindowProxy.isSupported()) {
    ...
    final void activityStoppedLocked(ActivityRecord r, Bundle icicle,
        ...
        /// M: activity state notifier @{
        mService.notifyActivityState(r.packageName, r.getFocusAppPid(),
                r.realActivity.getShortClassName(), IActivityStateNotifier.ActivityState.Stopped);
        /// @}
    ...
    private void completePauseLocked(boolean resumeNext) {
        ...
        if (prev != null) {
            ...
            Slog.d(TAG, "Delay resumeKeyDispatchingLocked() to avoid deadlock.");
    ...
    private void completeResumeLocked(ActivityRecord next) {
        /// M: Add for GMO launch time debug
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "amCompleteResume");
    ...
    private boolean isStackVisibleLocked() {
        ...
        /// M: BMW. [ALPS02515643]. BackFloating stack should be invisible @{
        if (MultiWindowProxy.isSupported()
    ...
    boolean isActivityOverHome(ActivityRecord record) {
        ...
        for (int taskNdx = mTaskHistory.indexOf(recordTask); taskNdx >= 0; --taskNdx) {
            ...
            for (int activityNdx = startNdx; activityNdx >= 0; --activityNdx) {
                final ActivityRecord r = activities.get(activityNdx);
                if (r.isHomeActivity()) {
                    if (DEBUG_VISIBILITY) Slog.v(TAG, "find home: " + r);
                    return true;
                }
                if (!r.finishing && r.fullscreen) {
                    // Passed activity is over a fullscreen activity.
                    if (DEBUG_VISIBILITY) Slog.v(TAG, "find fullscreen: " + r);
                    return false;
                }
            }
            if (task.isOverHomeStack()) {
                ...
                if (DEBUG_VISIBILITY) Slog.v(TAG, "find task over home: " + task);
                return true;
            }
        }
    ...
    private boolean skipStartActivityInEnsureVisibleLocked(ActivityRecord top,
        ...
        if (realHaltActivity) {
            if (DEBUG_VISIBILITY) Slog.d(TAG, "Skip by halt activity");
            return true;
        }
        /// @}
        /// M: ALPS01808519 @{
        if (mService.mStartedUsers.get(cur.userId) == null) {
            if (DEBUG_VISIBILITY) Slog.d(TAG, "Skip by not existing user " + cur.userId);
            return true;
        }
        /// @}
        /// M: ALPS01937222 @{
        if (PowerOffAlarmUtility.isAlarmBoot()) {
            if (DEBUG_VISIBILITY) Slog.d(TAG, "Skip by alarm boot");
            return true;
        }
        /// @}
                ...
                if (!behindFullscreen || r.mLaunchTaskBehind) {
                    ...
                    if (r.app == null || r.app.thread == null) {
                        ...
                        if (MultiWindowProxy.isSupported()
                            ...
                            /// M: BMW. If this activity is already visible, and doing restore
                            /// or max, then relaunch it to pause+stop (but remain visible).
                            relaunchActivityLocked(r, 0, false);
                            if (DEBUG_VISIBILITY) {
                                Slog.v(TAG, "[BMW] Restore/Max: already visible at "
                                        + r + ", and relaunch it to stop.");
                            }
                        } else {
                    ...
                    if (r.fullscreen) {
                        ...
                        behindFullscreen = true;
                    } else if (MultiWindowProxy.isSupported() && isFloatingStack()) {
                        /// M: BMW, behindFullscreen is determined by floatingOrTranslucent @{
                        behindFullscreen = !r.mFloating && !r.mTranslucent;
                        if (DEBUG_VISIBILITY) {
                            Slog.v(TAG, "[BMW] Full Float Window " + behindFullscreen);
                        }
                        /// @}
                    }
                } else {
    ...
    private boolean resumeTopActivityInnerLocked(ActivityRecord prev, Bundle options) {
        ...
        if (next == null) {
            ...
            if (MultiWindowProxy.isSupported()) {
                if (mStackSupervisor.moveNextFloatingStackToTopLocked(returnTaskType, this)) {
                    return true;
                }
            }
            /// @}
        ...
        /// M. BMW. [ALPS02194735] Restart if process in MinimaxRestartList hasn't
        /// clearup when restore @{
        boolean needResume = true;
        if (MultiWindowProxy.isSupported()) {
            boolean shouldRestart =
                    MultiWindowProxy.getInstance().shouldRestartWhenMiniMax(next.packageName);
            needResume = !shouldRestart || (shouldRestart && !next.mPendingRestoreMax);
        }
        /// @}
        ...
        if (next.app != null && next.app.thread != null && needResume) {
            ...
            try {
                ...
                if (MultiWindowProxy.isSupported() && next.mPendingRestoreMax) {
                    ...
                    ///  the activity not in WhiteList  should not be floating window.
                    /// normal A ->B, restore B to floating . Then B back to A,
                    /// A should not be floating window

                    /// M: [ALPS02071575] Check packagename if disbled for floating
                    // TODO: We should check why create floatingstack
                    /// for the ac(that not inWhiteList).
/*
                    if (!toMax && !MultiWindowProxy.getInstance().inWhiteList(next.packageName)) {
                        mStackSupervisor.moveFloatingStackToAppStackLocked(this, false);
                        toMax = true;
                    }
*/
                    if (!ActivityManagerService.IS_USER_BUILD) {
                        Slog.d(TAG, "[BMW] ACT-AM_RESTORE_ACTIVITY " + next
                                + " task:" + next.task.taskId);
                    }

                    next.scheduleRestoreActivity(next.results, next.newIntents, 0,
                            false , null, toMax);
                    mStackSupervisor.checkReadyForSleepLocked();
                } else {
                    ...
                    if (DEBUG_STATES) Slog.d(TAG_STATES,
                             "resumeTopActivityLocked: Resumed " + next);
                }
            } catch (Exception e) {
            ...
            /// M. BMW. [ALPS02194735] Cleanup before restart if process
            /// inimaxRestartList hasn't clearup @{
            if (MultiWindowProxy.isSupported()) {
                if (next.app != null && next.app.thread != null && next.mPendingRestoreMax
                        && MultiWindowProxy.getInstance()
                            .shouldRestartWhenMiniMax(next.packageName)) {
                    /// M: [ALPS02495931] Update process state before killing app
                    mStackSupervisor.updateProcessStatus(next);
                    mService.appDiedLocked(next.app);
                }
            }
            /// @}
            // Whoops, need to restart this activity!
            ...
            /// M. BMW. [ALPS02194735] Reset mPendingRestoreMax flag. @{
            if (MultiWindowProxy.isSupported()) {
                next.mPendingRestoreMax = false;
            }
            /// @}
    ...
    private void insertTaskAtTop(TaskRecord task, ActivityRecord newActivity) {
        ...
        if (isOnHomeDisplay()) {
            ...
            /// M: BMW. Check whether the task's background is Home.
            boolean backIsHome = false;

            /// M: BMW. If normal Ap is launched from Home, keep fromHome is true.@{
            if (MultiWindowProxy.isSupported()) {
                /// M: [ALPS02069449] & [ALPS02147336]. If lastStack is floating,
                /// and appStack is in resumed, set backIsHome false.
                if (!isHomeStack() && lastStack.isFloatingStack()) {
                    ActivityStack appStack = getAppStackLocked();
                    if (appStack == null || appStack.mResumedActivity == null) {
                        backIsHome = true;
                    }
                }
                if (backIsHome) {
                    fromHome = true;
                    if (DEBUG_TASK_RETURNTO) {
                        Slog.d(TAG, "[BMW] insertTaskAtTop set fromHome true");
                    }
                }
            }
            /// @}
            if (!isHomeStack() && (fromHome || topTask() != task)) {
                ...
                /// M: BMW. [ALPS02069449]. If activity is launched from home,
                /// set TaskToReturnTo home.@{
                if (MultiWindowProxy.isSupported()) {
                    if (backIsHome) {
        ...
        /// M:BMW. @{
        if (MultiWindowProxy.isSupported()) {
            // For floating stack, it's no need to care about its root task's taskToReturnTo.
            // And in current design, there is only one task in a floating stack,
            // so we don't need to consider multiple tasks.
            // TODO: to consider multiple tasks in a floating stack.
            if (isFloatingStack()) {
                task.setTaskToReturnTo(ActivityRecord.NULL_ACTIVITY_TYPE);
            }
        }
        /// @}
        ...
        for (int i = numActivities - 1; i > rootActivityNdx; --i) {
            ...
            if (target.resultTo != null) {
                ...
                if (forceReset || finishOnTaskLaunch) {
                    ...
                    if (!ActivityManagerService.IS_USER_BUILD) Slog.v(TAG_TASKS,
    ...
    private void adjustFocusedActivityLocked(ActivityRecord r, String reason) {
        if (mStackSupervisor.isFrontStack(this) && mService.mFocusedActivity == r) {
            ...
            if (next != r) {
                final TaskRecord task = r.task;
                /// M: BMW. [ALPS02464397]. Disable sticky before removing last activity @{
                if (MultiWindowProxy.isSupported() && isStickStack()) {
                    if (r.frontOfTask && task == topTask()) {
                        MultiWindowProxy.getInstance().stickWindow(r.appToken, false);
                    }
                }
                /// @}
                if (r.frontOfTask && task == topTask() && task.isOverHomeStack()) {
                    ...
                    if (MultiWindowProxy.isSupported()) {
                        if (isFloatingStack()) {
                            return;
                        } else if (mStackSupervisor.moveNextFloatingStackToTopLocked(
                                     task.getTaskToReturnTo(), this)) {
                            return;
                        }
                    }
                    /// @}
    ...
    final ActivityRecord finishCurrentActivityLocked(ActivityRecord r, int mode, boolean oomAdj) {
        ...
        if (canDestroyHere && (mode == FINISH_IMMEDIATELY
    ...
    private void removeActivityFromHistoryLocked(ActivityRecord r, String reason) {
        ...
        r.takeFromHistory();
        /// M: AMS log enhancement @{
        if (null != mStackListener && mStackSupervisor.isFrontStack(this)) {
            mStackListener.dumpStack(this);
        }
        /// @}
        removeTimeoutsForActivityLocked(r);
        ...
        if (task != null && task.removeActivity(r)) {
            ...
            if (mStackSupervisor.isFrontStack(this) && task == topTask() &&
                ...
                if (MultiWindowProxy.isSupported()) {
                    needResumeHome = !mStackSupervisor
                            .moveNextFloatingStackToTopLocked(task.getTaskToReturnTo(), this);
                    // M: [ALPS02454517]. Disable sticky before removing last activity
                    MultiWindowProxy.getInstance().stickWindow(r.appToken, false);
                }
                if (needResumeHome) {
                ...
                /// M. BMW. [ALPS02127806]. Make sure rebuild window layer  @{
                if (MultiWindowProxy.isSupported()) {
                    mWindowManager.prepareAppTransition(AppTransition.TRANSIT_NONE, false);
                }
                /// @}
    ...
    final boolean destroyActivityLocked(ActivityRecord r, boolean removeFromApp, String reason) {
        ...
        /// M: activity state notifier @{
        mService.notifyActivityState(r.packageName, r.getFocusAppPid(),
                r.realActivity.getShortClassName(), IActivityStateNotifier.ActivityState.Destroyed);
        /// @}
                ...
                if (r.app == app) {
                    ...
                    if (MultiWindowProxy.isSupported()) {
                        ...
                        if (needRestart) {
                            ...
                            if (DEBUG_CLEANUP) {
                                Slog.v(TAG_CLEANUP, "[BMW] Keep focus for Max/ Restore");
                            }
                        }
                    }
                    /// @}
                    if ((!r.haveState && !r.stateNotNeeded && !needRestart) || r.finishing) {
    ...
    final boolean moveTaskToBackLocked(int taskId) {
        ...
        /// M: BMW. [ALPS02069449], It is unnecessary for FloatingStack.
        if (!isHomeStack() && numValidTasks == 1 && !isFloatingStack()) {
        ...
        if (prevIsHome || (task == tr && canGoHome) || (numTasks <= 1 && isOnHomeDisplay())) {
            ...
            /// if this is floating stack, move floating stack to back, and resume top activity.
            /// if this is app stack, resume home & floating stack in order
            if (MultiWindowProxy.isSupported()) {
                if (isFloatingStack()) {
                    // For floating stack, no need to resume homestack.
                    // set task returnTo:NULL_ACTIVITY_TYPE
                    tr.setTaskToReturnTo(ActivityRecord.NULL_ACTIVITY_TYPE);
                    // [ALPS02441765] Pause back and resume top. @{
                    ...
                    // if this is app stack, resume home & floating stack in order
                    if (mStackSupervisor.moveNextFloatingStackToTopLocked(taskToReturnTo,
                            this)) {
                        return true;
                    }
                }
        ...
        if (r.app != null && r.app.thread != null) {
            try {
                ...
                if (stackChanges != 0) {
                    ...
                    /// M: ALPS02296973, mOverrideConfig can not be used in
                    /// no-mResizeable task @{
                    if (DEBUG_CONFIGURATION) {
                        Slog.v(TAG_CONFIGURATION, "mOverrideConfig can not be used");
                    }
                    r.app.thread.scheduleActivityConfigurationChanged(
                            r.appToken, null);
                }
    ...
    private boolean relaunchActivityLocked(ActivityRecord r, int changes, boolean andResume) {
        ...
        try {
            ...
            /// action. @{
            if (MultiWindowProxy.isSupported() && r.mPendingRestoreMax) {
    ...
    ActivityStack getNextFrontFloatingStackLocked() {
        ArrayList<ActivityStack> stacks = mStackSupervisor.getFrontFloatStacks(mDisplayId);
        if (stacks != null) {
            for (int i = stacks.size() - 1; i >= 0; --i) {
                ActivityStack stack = stacks.get(i);
                if (stack != this) {
    ...
    boolean adjustFocusToNextFrontFloatingStackLocked(ActivityStack inStack, String reason) {
        final ActivityStack stack = (inStack != null) ? inStack : getNextFrontFloatingStackLocked();
