...
class ResizingFrame {
    ...
    private boolean createSurfaceLocked() {
        ...
        //c.drawColor(mContext.getResources().getColor(
        // com.mediatek.internal.R.color.mw_resize_color));
    ...
    public void initBounds(WindowState win, TaskStack stack) {
        ...
        mStack = stack;
        /// M: [ALPS02532700] Fix Splite Mode initBounds issue @{
        if (MultiWindowProxy.isSplitMode()) {
            DisplayInfo displayInfo = mStack.getDisplayContent().getDisplayInfo();
            final int dw = displayInfo.logicalWidth;
            final int dh = displayInfo.logicalHeight;
            Rect floatRect = win.getFloatingRect(mService.getRotation(), dw, dh);
            mBounds.set(floatRect);
        } else {
