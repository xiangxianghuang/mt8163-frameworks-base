...
class WindowStateAnimator {
        ...
        if (!mLastHidden) {
            ...
            if (mSurfaceControl != null) {
                ...
                try {
                    ...
                    /// M: BMW. notify taskStack that the surface has hiden. @{
                    if (MultiWindowProxy.isSupported()) {
                        TaskStack tmpStack = getStack();
                        if (tmpStack != null)
                            tmpStack.onWinHiden(this);
                    }
                    /// @}
    ...
    boolean finishDrawingLocked() {
        final boolean startingWindow =
                mWin.mAttrs.type == WindowManager.LayoutParams.TYPE_APPLICATION_STARTING;
        if (WindowManagerService.DEBUG_STARTING_WINDOW && startingWindow) {
            Slog.v(TAG, "Finishing drawing window " + mWin + ": mDrawState="
                    + drawStateToString());
        }
        if (mDrawState == DRAW_PENDING) {
            /// M: Enable more window log.
            if (!WindowManagerService.IS_USER_BUILD || WindowManagerService.DEBUG_SURFACE_TRACE
                || WindowManagerService.DEBUG_ANIM || WindowManagerService.SHOW_TRANSACTIONS
    ...
    static class SurfaceTrace extends SurfaceControl {
        ...
        /// M: Enable log.
        private final static boolean logSurfaceTrace = true;
    ...
    SurfaceControl createSurfaceLocked() {
        ...
        if (mSurfaceControl == null) {
            if (WindowManagerService.DEBUG_ANIM || WindowManagerService.DEBUG_ORIENTATION)
            ...
            try {
                ...
                if (!WindowManagerService.IS_USER_BUILD
                    || WindowManagerService.SHOW_TRANSACTIONS
                    || WindowManagerService.SHOW_SURFACE_ALLOC) {
            ...
            try {
                ...
                try {
                    ...
                    /// M: BMW. Update position for Stack's background surface synchronously. @{
                    if (MultiWindowProxy.isSupported()) {
                        TaskStack tmpStack = getStack();
                        if (tmpStack != null)
                            tmpStack.onWinPositionChanged(this, left, top);
                    }
                    /// @}
    ...
    void computeShownFrameLocked() {
        ...
        ///M:[ALPS01777633]ClipRect should be reset no matter app transition animation exist or not
        mHasClipRect = false;
    ...
    void updateSurfaceWindowCrop(final boolean recoveringMemory) {
        ...
        if (!clipRect.equals(mLastClipRect)) {
            ...
            try {
                ...
                /// M: BMW. Update windowCrop for Stack's background surface synchronously. @{
                if (MultiWindowProxy.isSupported()) {
                    TaskStack tmpStack = getStack();
                    if (tmpStack != null)
                        tmpStack.onWinCropChanged(this, clipRect);
                }
                /// @}
            } catch (RuntimeException e) {
    ...
    void setSurfaceBoundariesLocked(final boolean recoveringMemory) {
        ...
        if (surfaceResized) {
            ...
            try {
                ...
                /// M: BMW. Update size for Stack's background surface synchronously. @{
                if (MultiWindowProxy.isSupported()) {
                    TaskStack tmpStack = getStack();
                    if (tmpStack != null)
                        tmpStack.onWinSizeChanged(this, width, height);
                }
                /// @}
            ...
            if (mSurfaceControl != null) {
                try {
                    ...
                    mSurfaceControl.setMatrix(
                            mDsDx * w.mHScale, mDtDx * w.mVScale,
                            mDsDy * w.mHScale, mDtDy * w.mVScale);
                    /// M: BMW. Update matrix for Stack's background surface synchronously. @{
                    if (MultiWindowProxy.isSupported()) {
                        TaskStack tmpStack = getStack();
                        if (tmpStack != null)
                            tmpStack.onWinMatrixChanged(this,
                                mDsDx * w.mHScale, mDtDx * w.mVScale,
                                mDsDy * w.mHScale, mDtDy * w.mVScale);
                    }
                    /// @}
                    ...
                    if (mLastHidden && mDrawState == HAS_DRAWN) {
                        if (WindowManagerService.SHOW_TRANSACTIONS)
                                WindowManagerService.logSurface(w, "SHOW (performLayout)", null);
        ...
        if (mSurfaceX != left || mSurfaceY != top) {
            ...
            try {
                ...
                if (MultiWindowProxy.isSupported()) {
                    TaskStack tmpStack = getStack();
                    if (tmpStack != null)
                        tmpStack.onWinPositionChanged(this, mWin.mFrame.left + left,
                                mWin.mFrame.top + top);
                }
                /// @}
    ...
    boolean performShowLocked() {
        ...
        if (mDrawState == READY_TO_SHOW && mWin.isReadyForDisplayIgnoringKeyguard()) {
            ...
            if (WindowManagerService.DEBUG_VISIBILITY ||
                    (WindowManagerService.DEBUG_STARTING_WINDOW &&
                    mWin.mAttrs.type == WindowManager.LayoutParams.TYPE_APPLICATION_STARTING)) {
    ...
    boolean showSurfaceRobustlyLocked() {
        try {
            if (mSurfaceControl != null) {
                ...
                /// M: BMW. notify taskStack that the surface has shown. @{
                if (MultiWindowProxy.isSupported()) {
                    TaskStack tmpStack = getStack();
                    if (tmpStack != null)
                        tmpStack.onWinShown(this);
                }
                /// @}
    ...
    void drawIfNeeded() {
        ...
        if (mSurfaceControl == null) {
            ...
            if (mSurface != null) {
                final int dw = (int) mSurfaceW;
                final int dh = (int) mSurfaceH;
                mDrawNeeded = false;
        ...
        AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... para) {
                ...
                Bitmap bmShot = SurfaceControl.screenshot(new Rect(),
                                    (int) mWin.mWinAnimator.mSurfaceW,
                                    (int) mWin.mWinAnimator.mSurfaceH, mSurfaceLayer, mSurfaceLayer,
                                        false, 0);
    ...
    void doCacheBitmapAsync(final View view) {
        if (mWin.mToken == null) return;
        ...
        AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... para) {
                ...
                if (mDrawState < HAS_DRAWN) {
                    Slog.d(TAG, "FSW doesn't catch. mDrawState=" + mDrawState);
                    return null;
                }
                final IBinder token = mWin.mToken.token;
                Trace.traceBegin(Trace.TRACE_TAG_WINDOW_MANAGER, "AsyncScreenshot");
                ...
                Trace.traceEnd(Trace.TRACE_TAG_WINDOW_MANAGER);
                Slog.i(TAG, "doCacheBitmap end, mToken =" + token);
                return null;
            }
        };
        task.execute();
    ...
    /// M: BMW @{
    TaskStack getStack() {
        AppWindowToken wtoken = mWin.mAppToken;
        if (wtoken == null)
            return null;
