...
final class InputMonitor implements InputManagerService.WindowManagerCallbacks {
    ...
    private void addInputWindowHandleLw(final InputWindowHandle inputWindowHandle,
        ...
        /// [ALPS02048767] Avoid null stack for non-default display
        if (MultiWindowProxy.isSupported()
                && child.getDisplayId() == Display.DEFAULT_DISPLAY
                && MultiWindowProxy.getInstance().isFloatingStack(child.getStack().mStackId)) {
            inputWindowHandle.isFloating = true;
            /// M: [ALPS01470880]. Only Application window should be modified.
