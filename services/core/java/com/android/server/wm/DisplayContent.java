...
class DisplayContent {
    ...
    void moveStack(TaskStack stack, boolean toTop) {
        if (!mStacks.remove(stack)) {
            ...
            Slog.e(TAG, "moving stack that was not added: " + stack, new Throwable());
        }
    ...
    int stackIdFromPoint(int x, int y) {
        /// M: BMW.[ALPS02532668] Find the visible, touch-deliverable
        /// window under the given point @{
        if (MultiWindowProxy.isSupported()) {
            ...
            for (int i = mWindows.size() - 1; i >= 0; i--) {
                ...
                if (win.mAppToken == null) {
                    ...
                    win.getTouchableRegion(tmpRegion);
                    if (tmpRegion.contains(x, y) || modal) {
                        return -1;
                    }
                } else {
                    win.getStackBounds(bounds);
                    if (bounds.contains(x, y)) {
                        return win.getStack().mStackId;
                    }
                }
            }
        }
