...
class FocusedStackFrame {
    ...
    void setVisibility(TaskStack stack) {
        /// M: BMW. No need focusedstackframe for floating window @{
        boolean notShowFocusedFrame = false;
        if (MultiWindowProxy.isSupported()
                && stack != null && !mEnableFocusedFrame
                && MultiWindowProxy.getInstance().isFloatingStack(stack.mStackId)) {
            notShowFocusedFrame = true;
        }
        /// @}
