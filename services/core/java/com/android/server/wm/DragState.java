...
class DragState {
    ...
    private WindowState getTouchedWinAtPointLw(float xf, float yf) {
        ...
        for (int i = N - 1; i >= 0; i--) {
            ...
            /// M: BMW, [ALPS02514963]. This is a goole issue. We should constrain
            /// "touch modal" behavior only for the window that has app token,
            /// it means it has activity stack. @{
            if (!(MultiWindowProxy.isSupported() && child.getAppToken() == null)) {
