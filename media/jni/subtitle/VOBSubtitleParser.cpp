...
#define LOG_TAG "VOBSubtitleParser"
#include <sys/stat.h>
#include <utils/Log.h>
...
namespace android
{
void VOBSubtitleParser::memset32(UINT32 *pu4Dst, UINT32 ui4Value, UINT32 z_l)
{
    for (; (z_l % 4) != 0; z_l--)
    {
        *pu4Dst++ = ui4Value;
...
void VOBSubtitleParser::vEvDvdspuDecodeD8888(UINT32 u4Width, UINT32 u4Height, UINT8 *pu1Src, UINT32 *pu4Dst, UINT32 *pu4Cpt, UINT32 u4Pitch)
{
...
VOBSubtitleParser(char *data, int size)
{
    ...
    m_fgParseFlag = false;
    for (int i=0;i < VOB_TMP_FILE_COUNT; i++)m_pvBitmapData[i] = MAP_FAILED;
...
stPrepareBitmapBuffer()
{
    ...
    {
        sprintf(filepath,"/mnt/subt/IdxSubBitmapBuffer_%d.tmp",i);
        m_iFd[i] = open(filepath, O_CREAT | O_RDWR | O_TRUNC, 0666);
